...
std::string fileloc="/home/wjm/sancov.txt";
std::ofstream outfile(fileloc.c_str(), std::ios_base::app);
...
public:
  ...
  bool instrumentModule(Module &M, DomTreeCallback DTCallback,
                        PostDomTreeCallback PDTCallback, LoopInfoCallback LICallback);

private:
  void createFunctionControlFlow(Function &F);
  void instrumentFunction(Function &F, DomTreeCallback DTCallback,
                          PostDomTreeCallback PDTCallback, LoopInfoCallback LICallback, int function_id);
  ...
  bool InjectCoverage(Function &F, ArrayRef<BasicBlock *> AllBlocks, ArrayRef<int> BlocksToFlag, ArrayRef<BasicBlock *> ExitingBlocks, ArrayRef<int> ExitingBlocksToFlag,
  ...
  void InjectCoverageAtBlock(Function &F, BasicBlock &BB, size_t Idx, int bbflag, int exitingflag,
                             bool IsLeafFunc = true, bool isExiting = false);
...
PreservedAnalyses SanitizerCoveragePass::run(Module &M,
  ...
  auto LICallback = [&FAM](Function &F) -> const LoopInfo * {
    return &FAM.getResult<LoopAnalysis>(F);
  };
  if (!ModuleSancov.instrumentModule(M, DTCallback, PDTCallback, LICallback))
...
static int getAndUpdateValue()
{
  int SanCovBlockID;
  std::string inputfileloc="/home/wjm/value.txt";
  std::ifstream inputfile(inputfileloc.c_str()); 
  inputfile>>SanCovBlockID;
  inputfile.close();
  std::ofstream outputfile(inputfileloc.c_str());
  int temp= SanCovBlockID+1;
  outputfile<<temp;
  outputfile.close();
  return SanCovBlockID;
}

bool ModuleSanitizerCoverage::instrumentModule(
    Module &M, DomTreeCallback DTCallback, PostDomTreeCallback PDTCallback, LoopInfoCallback LICallback) {
  ...
  auto res = M.getSourceFileName();
  int func_id = 0;
  //if( res.find("/js-parser-expr.c") != std::string::npos )
  {
    //outfile<<"*****Module Name****"<<res<<"\n";
    for (auto &F : M)
    {
      auto fname = F.getName().str();
      //if((fname.find("parseGetterSetter")!=std::string::npos) && (fname.find("ASTBuilder")!=std::string::npos))
      /*take C++ template into consideration */
      if((fname.find("parser_parse_") != std::string::npos) || (fname.find("parser_reparse_") != std::string::npos)) 
      {
        func_id = getAndUpdateValue();
        outfile <<func_id <<":: "<<demangle(fname)<<"\n";  
        //outfile <<"    "<<fname<<"\n";
        outfile<<std::flush;
        instrumentFunction(F, DTCallback, PDTCallback, LICallback, func_id);
        //func_id++;
        outfile << "\n";
      }
      /*
      else if((fname.find("parse")!=std::string::npos) && (fname.find("ASTBuilder")!=std::string::npos)) {
        outfile <<"xx"<<": "<<fname<<"\n";
        outfile<<std::flush;
        instrumentFunction(F, DTCallback, PDTCallback, -1);
        outfile << "\n";
      }
      */
...
void ModuleSanitizerCoverage::instrumentFunction(
    Function &F, DomTreeCallback DTCallback, PostDomTreeCallback PDTCallback, LoopInfoCallback LICallback, int function_id) {
  if (F.empty())
    return;
  ...
  const LoopInfo *LI = LICallback(F);
  bool IsLeafFunc = true;
  
  // normal BB for parse function: function_id
  // entry BB for parse function: 1<<25 | function_id
  // return BB for parse function: 1<<26 | function_id
  // ER BB for parse function: 1<<27 | function_id (entryBB and returnBB are the same)

  // LoopHeader for parse function: 1<<29 | loopID<<10 | function_id
  // Latch block for parse function: 1<<30 | loopID<<10 | function_id
  // Exiting block for parse function: 1<<24 | loopID<<10 | function_id

  // 标记所有的exiting block，在这些基本块的结尾插桩，与latch重合的exiting不需要标记，与header重合的可以标记
  // IRBuilder<> Builder(BB->getTerminator());
  // Builder.CreateCall(InstrFunction, {42, 2});
  // 只起到标记的作用，用于判断loop的结尾位置，最后出现的exiting即为循环最后一次迭代的结尾，中间出现的exiting不一定是循环结尾


  SmallVector<int,16> BlocksToFlag;
  SmallVector<int,16> ExitingBlocksToFlag;
  SmallVector<BasicBlock *, 16> ExitingBlocksToInstrument;
  SmallVector<BasicBlock *, 16> EntryBlocks;
  SmallVector<BasicBlock *, 16> ReturnBlocks;
  SmallVector<BasicBlock *, 16> ERBlocks;

  SmallVector<BasicBlock *, 16> LoopHeaders;
  SmallVector<BasicBlock *, 16> LoopLatches;
  SmallVector<BasicBlock *, 16> LoopExitings;
  SmallVector<int,16> LoopExitingID;
  
  BasicBlock* entryBB = &(F.getEntryBlock());
  DebugLoc EntryLoc;
  if(entryBB)
  {
    BasicBlock::iterator IP1 = entryBB->getFirstInsertionPt();
    EntryLoc = IP1->getDebugLoc();
    if (!EntryLoc)
      if (auto SP = F.getSubprogram())
        EntryLoc = DILocation::get(SP->getContext(), 0, 0, SP);
    if(EntryLoc)
      outfile<<"    func start line:"<<EntryLoc.getLine()<<" \n";
    for (BasicBlock &BBB : F)
    {
      if (isa<ReturnInst>(BBB.getTerminator())) // if return BB == entry BB, continue
      {   
          if(entryBB == &BBB)
          {
            ERBlocks.push_back(&BBB);
            outfile<<"    [ERBlock!]\n";
          }
          else {
            EntryBlocks.push_back(entryBB);
            ReturnBlocks.push_back(&BBB);
          }
          BasicBlock::iterator IP2 = BBB.getFirstInsertionPt();
          EntryLoc = IP2->getDebugLoc();
          if (!EntryLoc)
            if (auto SP = F.getSubprogram())
              EntryLoc = DILocation::get(SP->getContext(), 0, 0, SP);
          if(EntryLoc)
            outfile<<"    func return line:"<<EntryLoc.getLine()<<" \n";
      }
    }
  }

  if(ReturnBlocks.size() > 1 || ERBlocks.size() > 1)
  {
    outfile<<"    [ERROR]More than one ReturnBlocks\n";
    outfile<<std::flush;
  }

  if(ReturnBlocks.size() == 0 && ERBlocks.size() == 0)
  {
    outfile<<"    [ERROR]No Entry Block and Return Block\n";
    outfile<<std::flush;
  }
  ...
  {
    auto subLoops = (*i)->getLoopsInPreorder();
    ...
    for(auto *loop : subLoops)
    {
      ...
      { 
        ...
        BasicBlock* latchBB = loop->getHeader();
        LoopHeaders.push_back(BB);
        for(auto* lbb : loop->getBlocksVector())
        {
          if(loop->isLoopLatch(lbb))
          {
            latchBB = lbb;
            LoopLatches.push_back(lbb);
          }  
        }
        ...
        for(auto* lbb : loop->getBlocksVector())
        {
          if(loop->isLoopExiting(lbb) && lbb != latchBB)
          {
            exiting_num++;
            LoopExitings.push_back(lbb);
            LoopExitingID.push_back(tmp_loopID);
          }
        }
        outfile<<"    loopid:"<<tmp_loopID<<", exiting size:"<<exiting_num<<"\n";
        
        for(auto* tmpBB : LoopLatches)
        {
          if(BB == tmpBB)
          {
            outfile<<"    [ERROR]The exiting and the header are the same!"<<"\n";
            outfile<<"     Size of current loop: "<<loop->getNumBlocks()<<"\n\n";
          }
            
        }
        BasicBlock::iterator IP = BB->getFirstInsertionPt();
        if (BB == entryBB) {
          outfile<<"    [ERROR]The loop header and the function entry are the same!"<<"\n";
          if (auto *SP = F.getSubprogram())
        ...
        if(EntryLoc)
          outfile<<"    Loop line:"<<EntryLoc.getLine()<<" \n";
        tmp_loopID++;
      }
    } 
  }

  outfile<<"    LoopHeaders:  "<<LoopHeaders.size()<<"\n";
  outfile<<"    LoopLatches: "<<LoopLatches.size()<<"\n";
  outfile<<"    LoopExitings: "<<LoopExitings.size()<<"\n";
  outfile<<"    LoopExitingID: "<<LoopExitingID.size()<<"\n";

  // compute exiting blocks to instrument at tail
  for (auto &DBB : F) 
  {
    int ii = 0;
    for(auto* tBB : LoopExitings) 
    {
      if(tBB == &DBB)
      {
        ExitingBlocksToInstrument.push_back(&DBB);
        int loopID = LoopExitingID[ii];
        ExitingBlocksToFlag.push_back(function_id | loopID<<10 | 1<<24);
        outfile<<"    Instrument LoopExiting"<<loopID<<" BB!\n";
        break;
      }
      ii++;
    }
  ...
  for (auto &DBB : F) 
  {
    ...
    // determine the block flag
    for(auto* flag1BB : EntryBlocks) // EntryBlocks
    {
      if(flag1BB == &DBB)
      {
        myflag=1;
    ...
    if(myflag==0)
    {
      for(auto* flag2BB : ReturnBlocks) // ReturnBlocks
      {
        if(flag2BB == &DBB)
        {
          myflag=2;
    ...
    if(myflag==0)
    {
      for(auto* flag3BB : ERBlocks) // ERBlocks
      {
        if(flag3BB == &DBB)
        {
          myflag=3;
    ...
    {
      int loop_time = 0;
      for(auto* flag4BB : LoopHeaders) // LoopHeaders
      {
        loop_time++;
        if(flag4BB == &DBB)
        {
    ...
    {
      int loop_time2 = 0;
      for(auto* flag5BB : LoopLatches) // LoopLatches
      {
        loop_time2++;
        if(flag5BB == &DBB)
        {
    ...
    if(myflag == 1)
    {
      BlocksToInstrument.push_back(&DBB);
      outfile<<"    Instrument function entry BB!\n";
      BlocksToFlag.push_back(function_id | 1<<25);
    }
    else if(myflag == 2)
    {
      BlocksToInstrument.push_back(&DBB);
      outfile<<"    Instrument function return BB!\n";
      BlocksToFlag.push_back(function_id | 1<<26);
    }
    else if(myflag == 3)
    {
      BlocksToInstrument.push_back(&DBB);
      outfile<<"    Instrument function entry&return BB!\n";
      BlocksToFlag.push_back(function_id | 1<<27);
    }
    else if(myflag == 4)
    {
      BlocksToInstrument.push_back(&DBB);
      outfile<<"    Instrument LoopHeader"<<loopID<<" BB!\n";
      if(loopID==0)
        outfile<<"    [CRASH] loopID must not be zero!\n";
      BlocksToFlag.push_back(function_id | loopID<<10 | 1<<29);
    }
    else if(myflag == 5)
    {
      BlocksToInstrument.push_back(&DBB);
      outfile<<"    Instrument LoopLatch"<<loopID<<" BB!\n";
      if(loopID==0)
        outfile<<"    [CRASH] loopID must not be zero!\n";
      BlocksToFlag.push_back(function_id | loopID<<10 | 1<<30);
    }
    else if (shouldInstrumentBlock(F, &DBB, DT, PDT, Options)|| is_exiting) // TODO: include exitings
    {
      
      BlocksToInstrument.push_back(&DBB);
      BlocksToFlag.push_back(function_id);
    }
  ...
  InjectCoverage(F, BlocksToInstrument, BlocksToFlag, ExitingBlocksToInstrument, ExitingBlocksToFlag, IsLeafFunc);
...
bool ModuleSanitizerCoverage::InjectCoverage(Function &F,
                                             ArrayRef<BasicBlock *> AllBlocks,
                                             ArrayRef<int> BlocksToFlag,
                                             ArrayRef<BasicBlock *> ExitingBlocks,
                                             ArrayRef<int> ExitingBlocksToFlag,
                                             bool IsLeafFunc) {
  ...
  for (size_t i = 0, N = AllBlocks.size(); i < N; i++)
  {
    ...
    // find Exiting Blocks
    int exiting_flag = 0;
    for(size_t j=0; j<ExitingBlocks.size(); j++)
    {
      if(AllBlocks[i] == ExitingBlocks[j])
      {
        isExiting = true;
        exiting_flag = ExitingBlocksToFlag[j];
        break;
      }
    }
    InjectCoverageAtBlock(F, *AllBlocks[i], i, BlocksToFlag[i], exiting_flag, IsLeafFunc, isExiting);
  }
...
void ModuleSanitizerCoverage::InjectCoverageAtBlock(Function &F, BasicBlock &BB,
                                                    size_t Idx, int bbflag, int exitingflag,
                                                    bool IsLeafFunc, bool isExiting) {
  BasicBlock::iterator IP = BB.getFirstInsertionPt();
  ...
  if (Options.TracePCGuard) {
    ...
    IRB.CreateCall(SanCovTracePCGuard, {GuardPtr,ConstantInt::get(Int32Ty, bbflag)})->setCannotMerge();
    if(isExiting){
      InstrumentationIRBuilder IRC(BB.getTerminator());
      IRC.CreateCall(SanCovTracePCGuard, {GuardPtr,ConstantInt::get(Int32Ty, exitingflag)})->setCannotMerge();
    }
  }
  if (Options.Inline8bitCounters) {
