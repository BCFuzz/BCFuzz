...
public:
  ...
  ModuleSanitizerCoverage(Module &M, DomTreeCallback DTCallback,
                          ...
                          LoopInfoCallback LICallback,
                          const SanitizerCoverageOptions &Options,
                          const SpecialCaseList *Allowlist,
                          const SpecialCaseList *Blocklist)
      : M(M), DTCallback(DTCallback), PDTCallback(PDTCallback), LICallback(LICallback),
...
private:
  ...
  void instrumentFunction(Function &F, int function_id);
...
PreservedAnalyses SanitizerCoveragePass::run(Module &M,
  ...
  ModuleSanitizerCoverage ModuleSancov(M, DTCallback, PDTCallback, LICallback,
...
bool ModuleSanitizerCoverage::instrumentModule() {
  ...
  //if( res.find("/js-parser-expr.c") != std::string::npos )
  {
    ...
    for (auto &F : M)
    {
      ...
      //if((de_fname.find("ParsePropertyName") != std::string::npos))
      if((de_fname.find("ParserBase<v8::internal::Parser>::Parse") != std::string::npos)) 
      {
        outfile <<func_id <<":: "<<de_fname<<"\n";  
        //outfile <<"    "<<fname<<"\n";
        outfile<<std::flush;
...
void ModuleSanitizerCoverage::instrumentFunction(Function &F, int function_id) {
  if (F.empty())
    return;
