...
std::string fileloc="/home/wjm/js_experiment/sancov.txt";
...
bool ModuleSanitizerCoverage::instrumentModule(
  ...
  int func_id = 0;
  if( res.find("/JSParserImpl.cpp") != std::string::npos )
  {
    outfile<<"*****Module Name****"<<res<<"\n";
    for (auto &F : M)
    {
      auto fname = F.getName().str();
      auto de_fname = demangle(fname);
      //if((fname.find("parseGetterSetter")!=std::string::npos) && (fname.find("ASTBuilder")!=std::string::npos))
      /*take C++ template into consideration */
      if(((de_fname.find("JSParserImpl::parse") != std::string::npos) || (de_fname.find("JSParserImpl::reparse") != std::string::npos)) && (de_fname.find("JSParserImpl::parseVariableStatement") == std::string::npos)) 
      {
        outfile <<func_id <<":: "<<de_fname/*demangle(fname)*/<<"\n";  
        outfile <<"    "<<fname<<"\n";
        outfile<<std::flush;
