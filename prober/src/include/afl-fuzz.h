...
typedef struct afl_state {
  ...
  u8  *virgin_bits,                      /* Regions yet untouched by fuzzing */
      *token_path_bits,
      *trim_token_path_bits,
      *virgin_tmout,                    /* Bits we haven't seen in tmouts   */
      *virgin_crash;                    /* Bits we haven't seen in crashes  */
  
  u8 path_result_bits[parser_func_num];
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u32 probe_func;                       /* which parer function is being explored now*/
  u32 path_queued_items[parser_func_num];
  ...
  struct queue_entry *queue,            /* Fuzzing queue (linked list)      */
      *queue_cur, *path_queue_cur,                     /* Current offset within the queue  */
      *queue_top;                    /* Top of the list                  */
  struct queue_entry *path_queue[parser_func_num];
  struct queue_entry *path_queue_top[parser_func_num];

  // growing buf
  struct queue_entry **queue_buf;
  struct queue_entry **path_queue_buf[parser_func_num];
...
u8   trim_case_custom(afl_state_t *, struct queue_entry *q, u8 *in_buf,
                      struct custom_mutator *mutator, u8 is_token);
...
void add_to_path_queue(afl_state_t *, u8 *, u32, u8, int);
...
//void save_token(afl_state_t *, u8* myfn);
int compute_path(afl_state_t *);
void trim_compute_path(afl_state_t *);
#ifndef AFL_SHOWMAP
...
u8   trim_case(afl_state_t *, struct queue_entry *, u8 *, u8);
...
//u32    select_next_queue_entry_for_path(afl_state_t *afl);
