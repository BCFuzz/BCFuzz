...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    ACTF("[%d]Attempting dry run with '%s'...", idx, fn);
    ...
    int path_interesting = compute_path(afl);
    if(path_interesting)
    {
      for(int mm = 0; mm<parser_func_num; mm++)
        if(afl->path_result_bits[mm])
          add_to_path_queue(afl, use_mem, read_len, 0, mm);
    }
    printf("***path interesting: %d***\n", path_interesting);

    //printf("***execution result:%d***\n",res);
    //if(res == 0) // syntax and semantic valid
    //  save_token(afl, NULL);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  /* Queue directory for path seed*/
  tmp = alloc_printf("%s/path_queue", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  for(int k=0;k<parser_func_num;k++)
  {
    tmp = alloc_printf("%s/path_queue/%d", afl->out_dir, k);
    if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
    ck_free(tmp);
  }
