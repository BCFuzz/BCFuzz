...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (unlikely(afl->queue_cur->cal_failed) && !(afl->queue_cur->token_path)) {
  ...
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
    ...
    u8 res = 0;
    if(afl->queue_cur->token_path !=1)
      res = trim_case(afl, afl->queue_cur, in_buf, 0);
    else 
      res = trim_case(afl, afl->queue_cur, in_buf, 1);
    //printf("*********trim_case_end********\n");
    orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);
  ...
  //printf("**************seed before mutation*************\n");
  //printf("%s\n", out_buf);
  //printf("***********************************************\n");


  // get seed after trimming by wjm
   ...
   *********************************************/
   ...
   //if(afl->queue_cur->token_path)
    //printf("*****mutate syntax path seed********\n");
...
skip_interest:
  ...
  //FILE* token_fp2 = fopen(temp, "a");

  //printf("user extra insert begin\n");
  //fprintf(token_fp2, "user extra insert begin %d\n", len);
  //fflush(token_fp2);
  ...
  //fprintf(token_fp2, "user extra insert end\n");
  //fflush(token_fp2);
...
custom_mutator_stage:
   ...
   * CUSTOM MUTATORS *
   *******************/
  //if(afl->queue_cur->token_path){printf("*****mutate syntax path seed by token mutator****\n");}
...
static u8 mopt_common_fuzzing(afl_state_t *afl, MOpt_globals_t MOpt_globals) {
  ...
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
    ...
    u8 res = trim_case(afl, afl->queue_cur, in_buf, 0);
    orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);
