...
// Exiting block for parse function: 1<<30 | loopID<<10 | function_id

// Call_holder 1<<31 | function_id
// Loop_holder 1<<31 | loopID <<10 | function_id

u32 token_path[4096*10];
u32 line_path[4096*10];
char token_str[4096*10*16];
u32 token_loc=0;

u8 bc_visited[0x1000];
char bc_str[4096*8];

// TODO: manage afl_state token seed queue and seed scheduling
// FINISH: afl->token_path_bits 0x100000  && hash size = 23


void computeAndUpdateHashInternal(uint32_t* hash, uint32_t value)
{
    uint32_t hash2 = *hash;
    const int move_bit = 9;
    const int bbhash_bit_count = 20 + 3;
    hash2 = (hash2 << move_bit) | (hash2 >> (bbhash_bit_count - move_bit));
    hash2 &= (1<<bbhash_bit_count) - 1;
    hash2 ^= value & 0xFF;
    hash2 = (hash2 << move_bit) | (hash2 >> (bbhash_bit_count - move_bit));
    hash2 &= (1<<bbhash_bit_count) - 1;
    hash2 ^= (value>>8) & 0xFF;
    uint32_t remains = value >> 16;
    while(remains){
        hash2 = (hash2 << move_bit) | (hash2 >> (bbhash_bit_count - move_bit));
        hash2 &= (1<<bbhash_bit_count) - 1;
        hash2 ^= (remains) & 0xFF;
...
void clear_bc()
{
  memset(bc_visited, 0, 0x1000);
  sprintf(bc_str,"Bytecode:");
}

// get bc according to token_line
//bc_visited: if bytecode has been recorded in this token path
//bc_trace_bits
//bc_line_trace_bits
//bc_str

void get_bc(int token_line)
{
  if(token_line == 0 | token_line == 1) // block flag such as loop header, call or builtin bytecode
    return;
  
  int bc_len = bc_trace_bits[0];
  int bc_line_len = bc_line_trace_bits[0];

  if(bc_len!=bc_line_len)
    printf("[ERROR]bc len(%d) and bc line len(%d) are different!\n", bc_len, bc_line_len);

  if(bc_len>=4095)
    printf("[ERROR]bc len(%d) can not be bigger than 4095\n", bc_len);

  if(bc_len==0)
    return;

  // find token_line in bc_line
  int found = 0;
  int current_bc = -1;
  for(int i =1; i<=bc_len; i++)
  {
    if((int)bc_line_trace_bits[i] == token_line) // match line
    {
      found = 1;
      current_bc = bc_trace_bits[i];
      if(bc_visited[current_bc]==0) // avoid repetitive bytecode
      {
        bc_visited[current_bc] = 1;
        sprintf(bc_str, "%s %d", bc_str, current_bc);
      }
    }
  }
  if(found == 1)
  ...
  // not found token_line in bc_line, token_line-- until found token_line in bc_line
  for(int kk = token_line-1; kk>1; kk--)
  {
    for(int i =1; i<=bc_len; i++)
    {
      if((int)bc_line_trace_bits[i] == kk) // match line
      {
        found = 1;
        current_bc = bc_trace_bits[i];
        if(bc_visited[current_bc]==0)
        {
          bc_visited[current_bc] = 1;
          sprintf(bc_str, "%s %d", bc_str, current_bc);
        }
...
// 0 non insteresting
// 1 interesting
int function_is_not_black(int function_id)
{
  if(function_id == 3 || function_id == 28 || function_id == 30)
    return 0;
  return 1;
}

int compute_path(afl_state_t *afl)
{

  memset(afl->path_result_bits, 0, parser_func_num);
  memset(afl->interesting_token_path, 0, 200*sizeof(u32));
  afl->interesting_len = 0; // max:199
  ...
  {
    char temp[1024];
    sprintf(temp,"%s/token.txt", afl->out_dir);
    token_fp = fopen(temp, "a");
  }
  execute_time++;
  int len = token_trace_bits[0];
  int line_len = line_trace_bits[0];

  if(len != line_len)
    printf("[ERROR] Token len(%d) and line len(%d) are different!\n", len, line_len);

  //fprintf(token_fp, "Start compute path, len: %d\n", len);
  //fflush(token_fp);

  token_loc = 0;
  if(len == 0)
    return 0;
  if(token_trace_bits[len] == 1<<28)
    error_flag= 1;
  for(int i = 1; i<=len; i++)
  {
    u32 token_id = token_trace_bits[i];
    u32 line_id = line_trace_bits[i];
    token_path[token_loc] = token_id;
    line_path[token_loc] = line_id;
    token_loc++;
    if(token_loc>4095*10)
    {
      printf("token path is too long!\n");
      token_loc = 0;
      return 0;
    }
    if((token_id & (1<<26))!=0) // return BB
    {
      int function_id = token_id & 0xFF;
      int ll = 0;
      // identify path start
      for(ll = token_loc-1; ;ll--)
      {
        if((token_path[ll] & (1<<25))!=0 && (token_path[ll] & 0xFF)==function_id)
          break;
        if(ll==-1)
        {
          printf("Miss function entry block!\n");
          return 0;
        }
      }
      //compute hash and record token str
      u32 result = 0;
      sprintf(token_str,"");
      clear_bc();
      for(u32 k=ll; k<=token_loc-1; k++)
      {
        if((token_path[k] & (1<<24))==0) // exiting block does not hash
          computeAndUpdateHashInternal(&result, token_path[k]);
        sprintf(token_str, "%s %d-%d", token_str, token_path[k], line_path[k]);
        get_bc(line_path[k]);
        token_path[k] = 0;
        line_path[k] = 0;
      }
      if(error_flag)
      {
        computeAndUpdateHashInternal(&result, 1<<28);
        sprintf(token_str, "%s %d-%d", token_str, 1<<28, 0);
      }
      // update bitmap and save token path
      if(((afl->token_path_bits[result/8] & (1<<(result%8))) == 0 && function_is_not_black(function_id)) && afl->interesting_len < 200)
      {
        fprintf(token_fp, "%s\n", bc_str);
        fprintf(token_fp, "%s\n", token_str);
        afl->token_path_bits[result / 8] |= 1<<(result % 8);
        interesting = 1;
        afl->path_result_bits[function_id] = 1; // used for path queue

        afl->interesting_token_path[afl->interesting_len] = result; // record interesting path for trimming
        afl->interesting_len++;
        afl->token_path_num++;
      }
      // placeholder
      token_loc = ll;
      if(function_is_not_black(function_id)) {
        token_path[token_loc] = 1<<31 | function_id;
        line_path[token_loc] = 0;
        token_loc++;
      }
      
    }
    else if((token_id & (1<<30))!=0)// LoopExiting BB
    {
      int function_id = token_id & 0xFF;
      int loop_id = (token_id >>10) & 0xFF;
      if(loop_id == 0)
      {
        printf("loop id can not be zero!");
        return 0;
      }
      int ll = 0;
      // identify path start
      for(ll = token_loc-1; ;ll--)
      {
        if((token_path[ll] & (1<<29))!=0)
        {
          if(((token_path[ll]>>10)&0xFF) == loop_id )
            break;
        }
        if(ll==-1)
        {
          printf("Miss loop header block!\n");
          return 0;
        }
      }
      //compute hash and record token str
      ...
      if(((afl->token_path_bits[result/8] & (1<<(result%8))) == 0 && function_is_not_black(function_id)) && afl->interesting_len < 200)
      {
        ...
        afl->token_path_bits[result / 8] |= 1<<(result % 8);
        interesting = 1;
        afl->path_result_bits[function_id] = 1;
      ...
      // placeholder
      token_loc = ll;
      u32 loopholder  = 1<<31 | (loop_id<<10) | function_id;
      if(token_path[token_loc-1] != loopholder && function_is_not_black(function_id))
      {
        token_path[token_loc] = loopholder;
        line_path[token_loc] = 0;
        token_loc++;
      }    
    }
    else if((token_id & (1<<28))!=0)// Error marker
    {
      if(token_loc==1)
        return 0;
      int last_index =  token_loc-1;
      for(int ll = token_loc -1 ;ll>=0; ll--)
      {
        if((token_path[ll] & (1<<25)) == 1) // 1<<25 function entry
        {
          u32 result = 0;
          int function_id = token_path[ll] & 0xFF;
          sprintf(token_str,"");
          clear_bc();
          for(int k=ll; k<last_index; k++)
          {
            if((token_path[k] & (1<<24))==0) // exiting block does not hash
              computeAndUpdateHashInternal(&result, token_path[k]);
            sprintf(token_str, "%s %d-%d", token_str, token_path[k], line_path[k]);
        ...
        else if((token_path[ll] & (1<<29)) == 1) // 1<<29 loop header
        {
          u32 result = 0;
          int function_id = token_path[ll] & 0xFF;
          sprintf(token_str,"");
  ...
  //fprintf(token_fp, "End compute path, interesting: %d\n", interesting);
  //fflush(token_fp);
  return interesting;
}

void trim_compute_path(afl_state_t *afl)
{
  memset(afl->trim_token_path_bits, 0, 0x100000);
  int error_flag = 0;
  int len = token_trace_bits[0];
  ...
  for(int i = 1; i<=len; i++)
  {
    ...
    if((token_id & (1<<26))!=0) // return BB
    {
      ...
      // update bitmap and save token path
      if(((afl->trim_token_path_bits[result/8] & (1<<(result%8))) == 0 && function_is_not_black(function_id)))
      {
        afl->trim_token_path_bits[result / 8] |= 1<<(result % 8);
      }
      // placeholder
      token_loc = ll;
      if(function_is_not_black(function_id)) {
...
// open file based on function and loop id
/*
void save_token(afl_state_t *afl, u8* myfn)
{
  static int count = 0;
  if(count == 0){
    char temp[1024];
  ...
  if(len == 0)
    return;
  //char record[4096*10]="";
  //char tmp_str[4096*10]="";
  //sprintf(record, "%d", token_trace_bits[1]); // token_trace_bits: len token1 token2
  fprintf(token_fp, "%d", token_trace_bits[1]);
  for(int i=1; i<len; i++)
  {
    //sprintf(tmp_str, "%s", record);
    //sprintf(record, "%s %d", tmp_str, token_trace_bits[i+1]);
    fprintf(token_fp, " %d", token_trace_bits[i+1]);
  }
  fprintf(token_fp,"\n");
  if(myfn)
    fprintf(token_fp, "from %s\n\n", myfn);
  fflush(token_fp);
}
*/
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  if(compute_path(afl))
  {
    for(int mm = 0; mm<parser_func_num; mm++)
      if(afl->path_result_bits[mm])
        add_to_path_queue(afl, mem, len, 0, mm);
  }
