...
u8 trim_case_custom(afl_state_t *afl, struct queue_entry *q, u8 *in_buf,
  ...
  while (afl->stage_cur < afl->stage_max) {
    ...
    if (likely(retlen)) {
      ...
      if (unlikely(!retlen)) {
        ...
        // trim based on token_path
        else if(is_token == 1)
        {
          if(q->token_path !=1)
            FATAL("Token path based trimming is going to trim edge seed!\n");
          // compute path
          trim_compute_path(afl);

          // compare path: q->interesting_token_path <-> afl->trim_token_path_bits
          for(u32 i=0; i<q->interesting_len; i++)
          {
            u32 hash_id =  q->interesting_token_path[i];
            if((afl->trim_token_path_bits[hash_id / 8] & (1<<(hash_id % 8))) == 0)
            {
              token_trim_success = 0; // set flag
              break;
            }
          }
        }
    ...
    {
      if (likely(retlen && (token_trim_success == 1))) {
        if (!out_buf) {
          memcpy(afl->clean_trace_custom, afl->fsrv.trace_bits, afl->fsrv.map_size);
        }
        if (afl_realloc((void **)&out_buf, retlen) == NULL) {
          FATAL("can not allocate memory for trim");
        }
  ...
  //printf("**************seed before mutation, len:%d *************\n", out_len);
  //printf("%s\n", out_buf);
  //printf("***********************************************\n");
  
  if(q->token_path)
  {
    printf("[TRIM] Token_path num: %d\n", q->interesting_len);
    printf("[TRIM] Token path trimming finished!\n");
  }
