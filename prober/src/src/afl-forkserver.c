...
u32* token_trace_bits;
u32* line_trace_bits;
u32* bc_trace_bits;
u32* bc_line_trace_bits;
void clear_token_trace_bits()
{
  memset(token_trace_bits, 0, MAP_SIZE);
  memset(line_trace_bits, 0, MAP_SIZE);
  memset(bc_trace_bits, 0, 0x4000); // BC_SHM_SIZE 0x4000
  memset(bc_line_trace_bits, 0, 0x4000); // BC_SHM_SIZE 0x4000
}
...
fsrv_run_result_t __attribute__((hot)) afl_fsrv_run_target(
  ...
  //printf("child status: %d\n", fsrv->child_status);// 666 in jscexit -> 39424
  //if(fsrv->child_status == 39424)
  //  return FSRV_RUN_INVALID;
