...
u32 *afl_token_shm_init() 
{
  u8 *shm_str;
  int shm_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
  if (shm_id < 0) {
    PFATAL("token shmget() failed, try running afl-system-config");
  }
  shm_str = alloc_printf("%d", shm_id);
  setenv(SHM_ENV_VAR2, shm_str, 1);
  ck_free(shm_str);
  u32* token_trace_bits = shmat(shm_id, NULL, 0);
  if (token_trace_bits == (void *)-1 || !token_trace_bits) {
    shmctl(shm_id, IPC_RMID, NULL);  // do not leak shmem
    PFATAL("token shmat() failed");
  }
  return token_trace_bits;
}

// init line shm for token
u32 *afl_line_shm_init() 
{
  u8 *shm_str;
  int shm_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
  if (shm_id < 0) {
    PFATAL("line shmget() failed, try running afl-system-config");
  }
  shm_str = alloc_printf("%d", shm_id);
  setenv(SHM_ENV_VAR3, shm_str, 1);
  ck_free(shm_str);
  u32* line_trace_bits = shmat(shm_id, NULL, 0);
  if (line_trace_bits == (void *)-1 || !line_trace_bits) {
    shmctl(shm_id, IPC_RMID, NULL);  // do not leak shmem
    PFATAL("token shmat() failed");
  }
  return line_trace_bits;
}
...
{
  u8 *shm_str;
  int shm_id = shmget(IPC_PRIVATE, 0x4000, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
  if (shm_id < 0) {
    PFATAL("line shmget() failed, try running afl-system-config");
  }
  shm_str = alloc_printf("%d", shm_id);
  setenv(SHM_ENV_VAR4, shm_str, 1);
  ck_free(shm_str);
  u32* bc_trace_bits = shmat(shm_id, NULL, 0);
  if (bc_trace_bits == (void *)-1 || !bc_trace_bits) {
    shmctl(shm_id, IPC_RMID, NULL);  // do not leak shmem
    PFATAL("token shmat() failed");
  }
  return bc_trace_bits;
}

u32 *afl_bc_line_shm_init() 
{
  u8 *shm_str;
  int shm_id = shmget(IPC_PRIVATE, 0x4000, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
  if (shm_id < 0) {
  ...
  shm_str = alloc_printf("%d", shm_id);
  setenv(SHM_ENV_VAR5, shm_str, 1);
  ck_free(shm_str);
  u32* bc_line_trace_bits = shmat(shm_id, NULL, 0);
  if (bc_line_trace_bits == (void *)-1 || !bc_line_trace_bits) {
    shmctl(shm_id, IPC_RMID, NULL);  // do not leak shmem
    PFATAL("token shmat() failed");
  }
  return bc_line_trace_bits;
}
