...
int main(int argc, char **argv_orig, char **envp) {
  ...
  token_trace_bits = afl_token_shm_init();

  line_trace_bits = afl_line_shm_init();
  bc_trace_bits = afl_bc_shm_init();
  bc_line_trace_bits = afl_bc_line_shm_init();
  ...
  if (afl->q_testcase_max_cache_entries) {
    //printf("***initialize q_testcase_cache***\n");
  ...
  cull_queue(afl);
  // wjm finish: set q->favored in path_add_to_queue
  // wjm finish: path_add_to_queue - manage queue_entry metadata
  // wjm TODO: seed selection from edge queue and path queue
  // wjm finish: hook perform_dry_run and save_if_interesting 
  // wjm TODO: hook general function like trime_case based on q->path
  ...
  while (likely(!afl->stop_soon)) {
    if(get_cur_time() - afl->start_time > 1000*60*60*72) // 72h
      goto stop_fuzzing;
    cull_queue(afl);
        ...
        if (likely(afl->pending_favored && afl->smallest_favored >= 0)) {
          ...
          */
          ...
          if(poss < 40)
          {
            ...
            // select from path queue
            // select_next_queue_for_path
            
            while(!(afl->path_queue_top[afl->probe_func]))
            {
              (afl->probe_func)++;
              afl->probe_func = afl->probe_func % parser_func_num;
            }
              
            

            printf("*****select from path queue %d*****\n", afl->probe_func);
            do {
              afl->current_path_entry = rand_below(afl, afl->path_queued_items[afl->probe_func]);
            } while (unlikely(afl->current_path_entry >= afl->path_queued_items[afl->probe_func]));
            afl->queue_cur = afl->path_queue_buf[afl->probe_func][afl->current_path_entry];
            (afl->probe_func)++;
            afl->probe_func = afl->probe_func % parser_func_num;
