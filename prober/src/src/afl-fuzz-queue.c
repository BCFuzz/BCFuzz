...
/*
inline u32 select_next_queue_entry_for_path(afl_state_t *afl) {
  u32    s = rand_below(afl, afl->path_queued_items);
  return s;
}
*/
...
void add_to_path_queue(afl_state_t *afl, u8 *mem, u32 len, u8 passed_det, int function_id) {

  u8* queue_fn ="";
  queue_fn = alloc_printf("%s/path_queue/%d/id_%06u", afl->out_dir, function_id, afl->path_queued_items[function_id]);
  s32 fd = open(queue_fn, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
  ...
  q->is_ascii = 0;
  q->exec_us= 10000;
  q->interesting_len = afl->interesting_len;
  q->interesting_token_path = (u32 *)malloc(q->interesting_len*sizeof(u32));
  memcpy(q->interesting_token_path, afl->interesting_token_path, q->interesting_len*sizeof(u32));

  if (afl->path_queue_top[function_id]) {
    afl->path_queue_top[function_id] = q;
  } else {
    afl->path_queue[function_id] = afl->path_queue_top[function_id] = q;
  }
  ++(afl->path_queued_items[function_id]);
  struct queue_entry **path_queue_buf = (struct queue_entry **)afl_realloc(
      ((void **)&(afl->path_queue_buf[function_id])), afl->path_queued_items[function_id] * sizeof(struct queue_entry *));
  if (unlikely(!path_queue_buf)) { PFATAL("path queue alloc"); }
  path_queue_buf[afl->path_queued_items[function_id] - 1] = q;
  q->id = afl->path_queued_items[function_id] - 1;
  q->skipdet_e = (struct skipdet_entry *)ck_alloc(sizeof(struct skipdet_entry));
}
...
inline u8 *queue_testcase_get(afl_state_t *afl, struct queue_entry *q) {
  ...
  while (unlikely(
    ...
    printf("******realloc q_testcase_cache******\n");
