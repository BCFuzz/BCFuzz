...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->token_path_bits = ck_alloc(0x100000);
  memset(afl->token_path_bits, 0, 0x100000);
  afl->trim_token_path_bits = ck_alloc(0x100000);
  memset(afl->trim_token_path_bits, 0, 0x100000);
  afl->interesting_token_path = ck_alloc(sizeof(u32)*200);
  afl->interesting_len = 0;
  afl->token_path_num = 0;
