...
u8 trim_case(afl_state_t *afl, struct queue_entry *q, u8 *in_buf, u8 is_token) {
  ...
  if (afl->custom_mutators_count) {
    ...
    LIST_FOREACH(&afl->custom_mutator_list, struct custom_mutator, {
      //printf("*******custom trim found!**********\n");
      if (el->afl_custom_trim) {

        trimmed_case = trim_case_custom(afl, q, in_buf, el, is_token); 
        custom_trimmed = true;
    ...
    // use line trimming, not byte trimming
    if (custom_trimmed) {
