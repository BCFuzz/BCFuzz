...
struct cov_context {
    ...
    struct shmem_data* shmem_ir;
    
    struct line_data* shmem_line;
    uint64_t bitmap_size_line;

    uint8_t* virgin_bits_ir;
    uint64_t bitmap_size_ir;
    uint64_t num_edges_ir;
    uint32_t* ir_count;
...
int ir_compare_equal(struct cov_context*, uint32_t edge_id);
//int ir_compare_equal(struct cov_context*, uint32_t* edges, uint32_t num_edges);
