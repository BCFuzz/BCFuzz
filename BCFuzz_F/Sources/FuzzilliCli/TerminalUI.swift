...
class TerminalUI {
    ...
    func printStats(_ stats: Fuzzilli_Protobuf_Statistics, of fuzzer: Fuzzer) {
        ...
        let bc_preserve_testcase_rate: Double = fuzzer.bc_testcase_num > 0 ? Double(fuzzer.bc_preserve_num / fuzzer.bc_testcase_num) : 0.0
        let bc_preserve_edge_rate:Double = fuzzer.bc_testcase_num > 0 ? Double(fuzzer.bc_new_edge / fuzzer.bc_testcase_num) : 0.0
        ...
        IR Corpus Size:               \(fuzzer.corpus.size2)
        IR found:                     \(fuzzer.foundIR())
        Distance Corpus Size:         \(fuzzer.corpus.size3)
        BC valid testcase:            \(fuzzer.bc_testcase_num)
        BC preserve testcase:         \(fuzzer.bc_preserve_num) (\(String(format: "%.2f%%", bc_preserve_testcase_rate * 100))) 
        BC preserve and new edge:     \(fuzzer.bc_new_edge) (\(String(format: "%.2f%%", bc_preserve_edge_rate * 100)))
