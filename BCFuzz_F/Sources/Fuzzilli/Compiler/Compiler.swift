...
public class JavaScriptCompiler {
    ...
    private func compileStatement(_ node: StatementNode) throws {
        ...
        case .classDeclaration(let classDeclaration):
            ...
            for field in classDeclaration.fields {
                ...
                case .method(let method):
                    ...
                    if method.isStatic {
                        head = emit(BeginClassStaticStringMethod(methodName: method.name, parameters: parameters))
                    } else {
                        head = emit(BeginClassInstanceStringMethod(methodName: method.name, parameters: parameters))
                    }
                ...
                case .getter(let getter):
                    let head: Instruction
                    if getter.isStatic {
                        head = emit(BeginClassStaticStringGetter(propertyName: getter.name))
                    } else {
                        head = emit(BeginClassInstanceStringGetter(propertyName: getter.name))
                    }
                ...
                case .setter(let setter):
                    let head: Instruction
                    if setter.isStatic {
                        head = emit(BeginClassStaticStringSetter(propertyName: setter.name))
                    } else {
                        head = emit(BeginClassInstanceStringSetter(propertyName: setter.name))
                    }
        ...
        case .forOfLoop(let forOfLoop):
            ...
            let loopVar = emit(BeginForOfLoop(isAsync: false), withInputs: [obj]).innerOutput
            try enterNewScope {
                map(initializer.name, to: loopVar)
        ...
        case .objectExpression(let objectExpression):
            ...
            for field in objectExpression.fields {
                ...
                case .property(let property):
                    ...
                    switch key {
                    case .name(let name):
                        emit(ObjectLiteralAddStringProperty(propertyName: name), withInputs: inputs)
                    case .index(let index):
                ...
                case .method(let method):
                    ...
                    if case .name(let name) = method.key {
                        instr = emit(BeginObjectLiteralStringMethod(methodName: name, isAsync: false, parameters: parameters))
                    } else {
                        instr = emit(BeginObjectLiteralComputedMethod(parameters: parameters, isAsync: false), withInputs: [computedPropertyKeys.removeLast()])
                    }
                ...
                case .getter(let getter):
                    ...
                    let instr = emit(BeginObjectLiteralStringGetter(propertyName: name))
                    try enterNewScope {
                ...
                case .setter(let setter):
                    ...
                    let instr = emit(BeginObjectLiteralStringSetter(propertyName: name))
                    try enterNewScope {
                        var parameters = instr.innerOutputs
                        map("this", to: parameters.removeFirst())
                        mapParameters([setter.parameter], to: parameters)
