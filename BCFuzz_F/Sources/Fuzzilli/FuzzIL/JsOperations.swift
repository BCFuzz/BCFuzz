...
class LabelableOperation: JsOperation {
    var label: Label

    init(label: Label, numInputs: Int = 0, numOutputs: Int = 0, numInnerOutputs: Int = 0, firstVariadicInput: Int = -1, attributes: Attributes = [], requiredContext: Context = .javascript, contextOpened: Context = .empty) {
        self.label = label
        super.init(numInputs: numInputs, numOutputs: numOutputs, numInnerOutputs: numInnerOutputs, firstVariadicInput: firstVariadicInput, attributes: attributes, requiredContext: requiredContext, contextOpened: contextOpened)
        label.holder = self
    }
...
public class Label {
    public struct Associatability: OptionSet {
        ...
        public static let notAssociatable       = Associatability([])
        public static let breakAssociatable     = Associatability(rawValue: 1 << 0)
        public static let continueAssociatable  = Associatability(rawValue: 1 << 1)
        public static let allAssociatable       = notAssociatable.union(breakAssociatable).union(continueAssociatable)

        public func AssociatableJumpType() -> [JumpType] {
            var result: [JumpType] = []
            if self.contains(Associatability.breakAssociatable) {
                result.append(JumpType.Break)
            }
            if self.contains(Associatability.continueAssociatable) {
                result.append(JumpType.Continue)
            }
            return result
        }
    }
    ...
    var ID: UInt64
    let associatability: Associatability
    private var holder_: LabelableOperation?

    init(labelAssociatability: Associatability) {
        self.ID = 0
        self.associatability = labelAssociatability
    }

    static func newBreakAssociaitableLabel() -> Label {
        return Label(labelAssociatability: Associatability.breakAssociatable)
    }

    static func newAllAssociaitableLabel() -> Label {
        return Label(labelAssociatability: Associatability.allAssociatable)
    }

    var holder: LabelableOperation {
        get {
            assert(holder_ != nil)
            return holder_!
        }
    ...
    public func isHolderAlive() -> Bool {
        return holder_ != nil
    }

    public func unsetHolder() {
        holder_ = nil
    }

    private var referencers: [JumpToLabel] = []

    func bindable(_ referencer: JumpToLabel) -> Bool {
        switch (referencer.jumpType) {
            case .Break:
                return associatability.contains(Associatability.breakAssociatable)
            case .Continue:
                return associatability.contains(Associatability.continueAssociatable)
        }
    }

    func randomAssociatableJumpType() -> JumpType {
        chooseUniform(from: associatability.AssociatableJumpType())
    }

    @discardableResult
    func addReferencer(_ referencer: JumpToLabel) -> Bool {
        if (bindable(referencer) && !referencers.contains { $0 === referencer }) {
            referencers.append(referencer)
            return true
        }
        return false
    }

    func removeReferencer(_ referencer: JumpToLabel) {
        referencers.removeAll { $0 === referencer }
    }

    func hasReferencer() -> Bool {
        return referencers.count > 0
    }

    func inReferencer(_ findee: JumpToLabel) -> Bool {
        return referencers.contains { $0 === findee }
    }
}
...
//          BeginObjectLiteralStringGetter 'baz' -> v56
//              // v56 is the |this| object
//              ...
//          EndObjectLiteralStringGetter
...
final class ObjectLiteralAddStringProperty: JsOperation {
    override var opcode: Opcode { .objectLiteralAddStringProperty(self) }
...
// A numeric property (but not integer), for example `1.1: 42`,
final class ObjectLiteralAddNumericProperty: JsOperation {
    override var opcode: Opcode { .objectLiteralAddNumericProperty(self) }

    let propertyName: Double

    init(propertyName: Double) {
        self.propertyName = propertyName
        super.init(numInputs: 1, attributes: .isMutable, requiredContext: .objectLiteral)
    }
}
...
final class BeginObjectLiteralStringMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralStringMethod(self) }

    let methodName: String
    let isAsync: Bool

    init(methodName: String, isAsync: Bool, parameters: Parameters) {
...
// A numeric method, for example `2.2(a3, a4) {`
final class BeginObjectLiteralNumericMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralNumericMethod(self) }

    let methodName: Double
    let isAsync: Bool

    init(methodName: Double, isAsync: Bool, parameters: Parameters) {
        self.methodName = methodName
        self.isAsync = isAsync
        // First inner output is the explicit |this| parameter
        super.init(parameters: parameters, numInnerOutputs: parameters.count + 1, attributes: [.isBlockStart, .isMutable], requiredContext: .objectLiteral, contextOpened: [.javascript, .subroutine, .method])
...
final class BeginObjectLiteralComputedMethod: BeginAnySubroutine {
    ...
    init(parameters: Parameters, isAsync: Bool) {
        self.isAsync = isAsync
        // First inner output is the explicit |this| parameter
...
final class BeginObjectLiteralStringGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralStringGetter(self) }
...
// A computed getter, for example `get ["23" + 1]() {`
final class BeginObjectLiteralComputedGetter: BeginAnySubroutine {
    ...
    init() {
        // First input is computed getter name
        // First inner output is the explicit |this| parameter
...
// A numeric getter, for example `get 1.1() {` or  `get 2() {`
final class BeginObjectLiteralNumericGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralNumericGetter(self) }
...
final class BeginObjectLiteralStringSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralStringSetter(self) }
...
// A computed setter, for example `set ["23" + 1](value) {`
final class BeginObjectLiteralComputedSetter: BeginAnySubroutine {
    ...
    init() {
        // First input is computed setter name
        // First inner output is the explicit |this| parameter
...
// A numeric setter, for example `set 1.1(a5) {` or `set 2(a5) {`
final class BeginObjectLiteralNumericSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginObjectLiteralNumericSetter(self) }
...
//
//         BeginClassInstanceStringGetter -> v12
//             // v12 is the |this| object
//             ...
//         EndClassInstanceGetter
//         BeginClassInstanceStringSetter -> v18, v19
...
final class BeginClassInstanceStringMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceStringMethod(self) }
...
final class BeginClassInstanceComputedMethod: BeginAnySubroutine {
    ...
    init(parameters: Parameters) {
        // First input is computed method name
        // First inner output is the explicit |this| parameter
...
final class BeginClassInstanceNumericMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceNumericMethod(self) }

    let methodName: Double

    init(methodName: Double, parameters: Parameters) {
        self.methodName = methodName
        // First inner output is the explicit |this| parameter
        super.init(parameters: parameters, numInnerOutputs: parameters.count + 1, attributes: [.isMutable, .isBlockStart], requiredContext: .classDefinition, contextOpened: [.javascript, .subroutine, .method, .classMethod])
    }
...
final class BeginClassInstanceBigIntMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceBigIntMethod(self) }

    let methodName: Int64

    init(methodName: Int64, parameters: Parameters) {
        self.methodName = methodName
        // First inner output is the explicit |this| parameter
        super.init(parameters: parameters, numInnerOutputs: parameters.count + 1, attributes: [.isMutable, .isBlockStart], requiredContext: .classDefinition, contextOpened: [.javascript, .subroutine, .method, .classMethod])
    }
...
final class BeginClassInstanceStringGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceStringGetter(self) }
...
final class BeginClassInstanceComputedGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceComputedGetter(self) }
...
final class BeginClassInstanceNumericGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceNumericGetter(self) }
...
final class BeginClassInstanceStringSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceStringSetter(self) }
...
final class BeginClassInstanceComputedSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceComputedSetter(self) }
...
final class BeginClassInstanceNumericSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassInstanceNumericSetter(self) }
...
final class BeginClassStaticStringMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticStringMethod(self) }
...
final class BeginClassStaticNumericMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticNumericMethod(self) }
...
final class BeginClassStaticBigIntMethod: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticBigIntMethod(self) }
...
final class BeginClassStaticStringGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticStringGetter(self) }
...
final class BeginClassStaticComputedGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticComputedGetter(self) }
...
final class BeginClassStaticNumericGetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticNumericGetter(self) }
...
final class BeginClassStaticStringSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticStringSetter(self) }
...
final class BeginClassStaticComputedSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticComputedSetter(self) }
...
final class BeginClassStaticNumericSetter: BeginAnySubroutine {
    override var opcode: Opcode { .beginClassStaticNumericSetter(self) }
...
final class ClassAddPrivateInstancePropertyWithPrivateInInitializer: JsOperation {
    override var opcode: Opcode { .classAddPrivateInstancePropertyWithPrivateInInitializer(self) }

    let propertyName: String
    let testInPropertyName: String
    var hasCustomInObject: Bool {
        return numInputs == 1
    }

    init(propertyName: String, testInPropertyName: String, hasCustomInObject: Bool) {
        self.propertyName = propertyName
        self.testInPropertyName = testInPropertyName
        super.init(numInputs: hasCustomInObject ? 1 : 0, requiredContext: .classDefinition)
    }
}
...
final class ClassAddPrivateStaticPropertyWithPrivateInInitializer: JsOperation {
    override var opcode: Opcode { .classAddPrivateStaticPropertyWithPrivateInInitializer(self) }
...
final class TestClassPrivateInInMethod: JsOperation {
    override var opcode: Opcode { .testClassPrivateInInMethod(self) }

    let propertyName: String

    init(propertyName: String) {
        self.propertyName = propertyName
        super.init(numInputs: 1, numOutputs: 1, requiredContext: .classMethod)
    }
}
...
class EndAnySubroutine: JsOperation {
    init(numInputs: Int = 0) {
        super.init(numInputs: numInputs, attributes: [.isBlockEnd])
    }
}
...
final class BeginExpressionBodyArrowFunction: BeginAnyFunction {
    override var opcode: Opcode { .beginExpressionBodyArrowFunction(self) }
}
final class EndExpressionBodyArrowFunction: EndAnyFunction {
    override var opcode: Opcode { .endExpressionBodyArrowFunction(self) }
...
public enum BinaryOperator: String, CaseIterable {
    ...
    case Exp      = "**"
    case UnRShift = ">>>"
    case NilCoals = "??"    // Nullish Coalescing
...
final class BeginWith: LabelableOperation {
    override var opcode: Opcode { .beginWith(self) }

    init(label: Label) {
        super.init(label: label, numInputs: 1, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .with])
        
    }

    convenience init() {
        self.init(label: Label.newBreakAssociaitableLabel())
    }
}
...
final class BeginWhileLoopBody: LabelableOperation {
    override var opcode: Opcode { .beginWhileLoopBody(self) }

    init(label: Label) {
        super.init(label: label, numInputs: 1, attributes: [.isBlockStart, .isBlockEnd, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }

    convenience init() {
        self.init(label: Label.newAllAssociaitableLabel())
    }
}
...
final class BeginDoWhileLoopBody: LabelableOperation {
    override var opcode: Opcode { .beginDoWhileLoopBody(self) }

    init(label: Label) {
        super.init(label: label, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }
...
final class BeginForLoopBody: LabelableOperation {
    ...
    init(label: Label, numLoopVariables: Int) {
        super.init(label: label, numInnerOutputs: numLoopVariables, attributes: [.isBlockStart, .isBlockEnd, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }

    convenience init(numLoopVariables: Int) {
        self.init(label: Label.newAllAssociaitableLabel(), numLoopVariables: numLoopVariables)
    }
}
...
final class BeginForInLoop: LabelableOperation {
    override var opcode: Opcode { .beginForInLoop(self) }

    init(label: Label) {
        super.init(label: label, numInputs: 1, numInnerOutputs: 1, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }
...
final class BeginForOfLoop: LabelableOperation {
    override var opcode: Opcode { .beginForOfLoop(self) }

    let isAsync: Bool

    init(label: Label, isAsync: Bool) {
        self.isAsync = isAsync
        super.init(label: label, numInputs: 1, numInnerOutputs: 1, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }

    convenience init(isAsync: Bool) {
        self.init(label: Label.newAllAssociaitableLabel(), isAsync: isAsync)
    }
}

final class BeginForOfLoopWithDestruct: LabelableOperation {
    override var opcode: Opcode { .beginForOfLoopWithDestruct(self) }

    let indices: [Int64]
    let hasRestElement: Bool

    init(label: Label, indices: [Int64], hasRestElement: Bool) {
        assert(indices.count >= 1)
        self.indices = indices
        self.hasRestElement = hasRestElement
        super.init(label: label, numInputs: 1, numInnerOutputs: indices.count, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }

    convenience init(indices: [Int64], hasRestElement: Bool) {
        self.init(label: Label.newAllAssociaitableLabel(), indices: indices, hasRestElement: hasRestElement)
    }
}
...
final class BeginRepeatLoop: LabelableOperation {
    ...
    init(label: Label, iterations: Int, exposesLoopCounter: Bool = true) {
        self.iterations = iterations
        super.init(label: label, numInnerOutputs: exposesLoopCounter ? 1 : 0, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.javascript, .loop])
    }

    convenience init(iterations: Int, exposesLoopCounter: Bool = true) {
        self.init(label: Label.newAllAssociaitableLabel(), iterations: iterations, exposesLoopCounter: exposesLoopCounter)
    }
}
...
final class BeginTry: JsOperation {
    ...
    init() {
        super.init(attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: [.tryBlock])
    }
}
...
final class BeginCatchWithDestruct: JsOperation {
    override var opcode: Opcode { .beginCatchWithDestruct(self) }

    let destructPropertyNames: [String]

    init(destructPropertyNames: [String] = []) {
        self.destructPropertyNames = destructPropertyNames
        super.init(numInnerOutputs: destructPropertyNames.count, attributes: [.isBlockStart, .isBlockEnd, .propagatesSurroundingContext])
    }
}
...
final class BeginBlockStatement: LabelableOperation {
    override var opcode: Opcode { .beginBlockStatement(self) }

    init(label: Label) {
        super.init(label: label, attributes: [.isBlockStart, .propagatesSurroundingContext], contextOpened: .javascript)
    }
...
///
final class BeginSwitch: LabelableOperation {
    override var opcode: Opcode { .beginSwitch(self) }

    init(label: Label) {
        super.init(label: label, numInputs: 1, attributes: [.isBlockStart], contextOpened: [.switchBlock])
    }
...
final class JumpToLabel: JsOperation {
    override var opcode: Opcode { .jumpToLabel(self) }

    var label: Label
    let jumpType: JumpType

    init(jumpType: JumpType, label: Label) {
        self.jumpType = jumpType
        self.label = label
        super.init(attributes: .isJump, requiredContext: [.javascript])
        assert(label.bindable(self))
        label.addReferencer(self)
    }

    public func destroy() {
        label.removeReferencer(self)
    }
