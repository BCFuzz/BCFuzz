...
public struct JSTyper: Analyzer {
    ...
    private mutating func processTypeChangesAfterScopeChanges(_ instr: Instruction) {
        ...
        func analyzeBinaryOperation(operator op: BinaryOperator, withInputs inputs: ArraySlice<Variable>) -> ILType {
            ...
            case .NilCoals:
                let LHSType = state.type(of: inputs[0])
                let RHSType = state.type(of: inputs[1])
                if LHSType.Is(.nullish) { // LHS is definitely only nullish
                    return RHSType
                } else if !LHSType.subsumes(.nullish) { // LHS mustn't be nullish
                    return LHSType
                } else { // LHS might be nullish
        ...
        case .beginCatchWithDestruct:
            instr.innerOutputs.forEach { set($0, .anything) }
