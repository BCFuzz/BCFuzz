...
public final class Program {
    ...
    public var line = 0
    public var interesting_index = -1
    public var origin = -1
...
public class LabelMapper {
    private var storage: Dictionary<UInt64, Label> = [:]

    public func has(_ ID: UInt64) -> Bool {
        return storage.contains { $0.key == ID }
    }

    public func get(_ ID: UInt64) -> Label {
        assert(has(ID))
        return storage[ID]!
    }

    public func set(_ ID: UInt64, _ label: Label) {
        assert(!has(ID))
        storage[ID] = label
    }

    func create(_ ID: UInt64, _ associatability: Label.Associatability) -> Label {
        assert(!has(ID))
        let label = Label(labelAssociatability: associatability)
        label.ID = ID
        set(ID, label)
        return label
    }
...
extension Program: ProtobufConvertible {
    ...
    func asProtobuf(opCache: OperationCache? = nil) -> ProtobufType {
        return ProtobufType.with {
            ...
            $0.uuid = id.uuidData
            $0.code = code.map({ $0.asProtobuf(labelableOperationCounter: &labelableOperationCounter, with: opCache) })
    ...
    convenience init(from proto: ProtobufType, opCache: OperationCache? = nil) throws {
        ...
        for (i, protoInstr) in proto.code.enumerated() {
            do {
                code.append(try Instruction(from: protoInstr, labelMapper: labelMapper, with: opCache))
