...
struct SimplifyingReducer: Reducer {
    ...
    func reduce2(_ code: inout Code, with helper: MinimizationHelper) {
        ...
        simplifyGuardedInstructions2(&code, with: helper)
    ...
    func simplifyFunctionDefinitions2(_ code: inout Code, with helper: MinimizationHelper) {
        ...
        for group in code.findAllBlockGroups() {
            ...
            // The resulting code may be invalid as we may be changing the context inside the body (e.g. turning an async function into a plain one).
            helper.tryReplacements2([(group.head, newBegin), (group.tail, newEnd)], in: &code, expectCodeToBeValid: false)
        }
    }
    ...
    func simplifyGuardedInstructions2(_ code: inout Code, with helper: MinimizationHelper) {
        // This will attempt to turn guarded operations into unguarded ones.
        // In the lifted JavaScript code, this would turn something like `try { o.foo(); } catch (e) {}` into `o.foo();`
        for instr in code {
            var newOp: Operation? = nil
