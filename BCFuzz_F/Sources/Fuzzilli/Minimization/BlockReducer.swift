...
struct BlockReducer: Reducer {
    func reduce(_ code: inout Code, with helper: MinimizationHelper) {
        ...
        for group in code.findAllBlockGroups() {
            ...
            case .beginClassConstructor,
                 ...
                 .beginClassInstanceStringGetter,
                 .beginClassInstanceComputedGetter,
                 .beginClassInstanceNumericGetter,
                 .beginClassInstanceStringSetter,
                 .beginClassInstanceComputedSetter,
                 ...
                 .beginClassStaticStringGetter,
                 .beginClassStaticComputedGetter,
                 .beginClassStaticNumericGetter,
                 .beginClassStaticStringSetter,
                 .beginClassStaticComputedSetter,
    ...
    func reduce2(_ code: inout Code, with helper: MinimizationHelper) {
        ...
        for group in code.findAllBlockGroups() {
            ...
            case .beginObjectLiteral:
                assert(group.numBlocks == 1)
                reduceObjectLiteral2(group.block(0), in: &code, with: helper)
            ...
            case .beginObjectLiteralStringMethod,
                 ...
                 .beginObjectLiteralNumericSetter:
                assert(group.numBlocks == 1)
                reduceFunctionInObjectLiteral2(group.block(0), in: &code, with: helper)

            case .beginClassDefinition:
                reduceClassDefinition2(group.block(0), in: &code, with: helper)
            ...
            case .beginClassConstructor,
                 ...
                 .beginClassPrivateInstanceMethod,
                 .beginClassPrivateStaticMethod:
                reduceFunctionInClassDefinition2(group.block(0), in: &code, with: helper)
            ...
            case .beginWhileLoopHeader,
                 ...
                 .beginForOfLoopWithDestruct,
                 .beginRepeatLoop:
                reduceLoop2(group, in: &code, with: helper)

            case .beginIf:
                reduceIfElse2(group, in: &code, with: helper)

            case .beginTry:
                reduceTryCatchFinally2(tryCatch: group, in: &code, with: helper)

            case .beginSwitch:
                reduceBeginSwitch2(group, in: &code, with: helper)
            ...
            case .beginWith:
                reduceGenericBlockGroup2(group, in: &code, with: helper)
            ...
            case .beginPlainFunction,
                 ...
                 .beginAsyncGeneratorFunction,
                 .beginConstructor:
                reduceFunctionOrConstructor2(group, in: &code, with: helper)

            case .beginCodeString:
                reduceCodeString2(group, in: &code, with: helper)
    ...
    private func reduceObjectLiteral2(_ literal: Block, in code: inout Code, with helper: MinimizationHelper) {
        // The instructions in the body of the object literal aren't valid outside of
        // object literals, so either remove the entire literal or nothing.
        helper.tryNopping2(literal.allInstructions, in: &code)
    }
    ...
    private func reduceFunctionInObjectLiteral2(_ function: Block, in code: inout Code, with helper: MinimizationHelper) {
        // The instruction in the body of these functions aren't valid inside the object literal as
        // they require .javascript context. So either remove the entire function or nothing.
        helper.tryNopping2(function.allInstructions, in: &code)
    }
    ...
    private func reduceClassDefinition2(_ definition: Block, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[definition.head].op is BeginClassDefinition)
        assert(code[definition.tail].op is EndClassDefinition)

        // Similar to the object literal case, the instructions in the body aren't valid outside of it, so remove everything.
        if helper.tryNopping2(definition.allInstructions, in: &code) {
            return
        }
        ...
        // Code reordering can change the numbering of variables, so they need to be renumbered.
        // The resulting code may also not be valid since we're moving code out of a method definition.
        helper.tryReplacements2(replacements, in: &code, renumberVariables: true, expectCodeToBeValid: false)
    }
    ...
    private func reduceFunctionInClassDefinition2(_ function: Block, in code: inout Code, with helper: MinimizationHelper) {
        // Similar to the object literal case, the instructions inside the function body aren't valid inside
        // the surrounding class definition, so we can only try to temove the entire function.
        helper.tryNopping2(function.allInstructions, in: &code)
    }
    ...
    private func reduceLoop2(_ loop: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        // We reduce loops by removing the loop itself as well as
        // any 'break' or 'continue' instructions in the loop body.
        var candidates = loop.blockInstructionIndices
        var inNestedLoop = false
        ...
        helper.tryNopping2(candidates, in: &code)
    }
    ...
    private func reduceIfElse2(_ group: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[group.head].op is BeginIf)
        assert(code[group.tail].op is EndIf)

        // First try to remove the entire if-else block but keep its content.
        if helper.tryNopping2(group.blockInstructionIndices, in: &code) {
            return
        }
        ...
        if group.numBlocks == 2 {
            ...
            let elseBlock = group.block(1)
            let rangeToNop = Array(elseBlock.head ..< elseBlock.tail)
            if helper.tryNopping2(rangeToNop, in: &code) {
                return
            }
            ...
            // ... as well as the BeginElse.
            replacements.append((elseBlock.head, Instruction(Nop())))
            helper.tryReplacements2(replacements, in: &code)
        }
    }
    ...
    private func reduceGenericBlockGroup2(_ group: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        var candidates = group.blockInstructionIndices
        if helper.tryNopping2(candidates, in: &code) {
            // Success!
            return
        }
    ...
    private func reduceBeginSwitch2(_ group: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[group.head].op is BeginSwitch)
        ...
        for block in blocks {
            // (3) Try to remove the cases here.
            helper.tryNopping2(block.allInstructions, in: &code)
        }
    }
    ...
    private func reduceFunctionOrConstructor2(_ function: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[function.head].op is BeginAnySubroutine)
        assert(code[function.tail].op is EndAnySubroutine)
        ...
        // So that the calls to the function can be removed by a subsequent reducer if only the body is important.
        // But its likely not worth the effort as the InliningReducer will do a better job at solving this.
        reduceGenericBlockGroup2(function, in: &code, with: helper)
    }
    ...
    private func reduceCodeString2(_ codestring: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[codestring.head].op is BeginCodeString)
        assert(code[codestring.tail].op is EndCodeString)
        ...
        replacements.append((codestring.head, Instruction(LoadString(value: ""), output: code[codestring.head].output)))
        replacements.append((codestring.tail, Instruction(Nop())))
        if helper.tryReplacements2(replacements, in: &code) {
            // Success!
            return
        }

        // If unsuccessful, default to generic block reduction
        reduceGenericBlockGroup2(codestring, in: &code, with: helper)
    }
    ...
    private func reduceTryCatchFinally2(tryCatch: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        assert(code[tryCatch.head].op is BeginTry)
        assert(code[tryCatch.tail].op is EndTryCatchFinally)
        ...
        if removedLastTryBlockInstruction && helper.tryNopping2(candidates, in: &code) {
            return
        }
        ...
        // Finally, fall back to generic block group reduction, which will attempt to remove the
        // entire try-catch block including its content
        reduceGenericBlockGroup2(tryCatch, in: &code, with: helper)
    }
}
