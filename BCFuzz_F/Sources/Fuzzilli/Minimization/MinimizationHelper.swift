...
class MinimizationHelper {
    ...
    func test2(_ code: Code, expectCodeToBeValid: Bool = true, numExecutions: Int = defaultNumExecutions) -> Bool {
        assert(numExecutions > 0)
        ...
        performOnFuzzerQueue {
            for _ in 0..<numExecutions {
                let execution = fuzzer.execute(Program(with: code), withTimeout: fuzzer.config.timeout * 2, purpose: .minimization)
                stillHasAspects = fuzzer.evaluator.hasAspects2(execution, aspects)
                guard stillHasAspects else { break }
            }
        }
        ...
        if stillHasAspects {
            didReduce = true
            self.myline = fuzzer.evaluator.get_line(aspects)
        } else {
            failedReductions += 1
        }
    ...
    @discardableResult
    func tryReplacing2(instructionAt index: Int, with newInstr: Instruction, in code: inout Code, expectCodeToBeValid: Bool = true, numExecutions: Int = defaultNumExecutions) -> Bool {
        assert(code[index].allOutputs == newInstr.allOutputs)
        ...
        let result = test2(code, expectCodeToBeValid: expectCodeToBeValid, numExecutions: numExecutions)
    ...
    @discardableResult
    func tryInserting2(_ newInstr: Instruction, at index: Int, in code: inout Code, expectCodeToBeValid: Bool = true, numExecutions: Int = defaultNumExecutions) -> Bool {
        // Inserting instructions will invalidate the instructionsToKeep list, so that list must be empty here.
        assert(instructionsToKeep.isEmpty)
        ...
        let result = test2(newCode, expectCodeToBeValid: expectCodeToBeValid, numExecutions: numExecutions)
    ...
    @discardableResult
    func tryNopping2(instructionAt index: Int, in code: inout Code, numExecutions: Int = defaultNumExecutions) -> Bool {
        return tryReplacing2(instructionAt: index, with: nop(for: code[index]), in: &code, expectCodeToBeValid: false, numExecutions: numExecutions)
    }
    ...
    @discardableResult
    func tryReplacements2(_ replacements: [(Int, Instruction)], in code: inout Code, renumberVariables: Bool = false, expectCodeToBeValid: Bool = true, numExecutions: Int = defaultNumExecutions) -> Bool {
        let originalCode = code
    ...
    @discardableResult
    func tryReplacing2(range: ClosedRange<Int>, in code: inout Code, with newCode: [Instruction], renumberVariables: Bool = false, expectCodeToBeValid: Bool = true, numExecutions: Int = defaultNumExecutions) -> Bool {
        assert(range.count >= newCode.count)
        ...
        return tryReplacements2(replacements, in: &code, renumberVariables: renumberVariables, expectCodeToBeValid: expectCodeToBeValid, numExecutions: numExecutions)
    }
    ...
    @discardableResult
    func tryNopping2(_ indices: [Int], in code: inout Code, expectCodeToBeValid: Bool = false) -> Bool {
        var replacements = [(Int, Instruction)]()
        for index in indices {
            replacements.append((index, nop(for: code[index])))
        }
        return tryReplacements2(replacements, in: &code, expectCodeToBeValid: false)
    }
...
protocol Reducer {
    ...
    func reduce2(_ code: inout Code, with tester: MinimizationHelper)
}
