...
public class ProgramBuilder {
    ...
    /// Stack of active try-catch-finally blocks.
    private var activeTryCatchFinallyBlock = Stack<TryCatchFinallyBlock>()

    public var currentTryCatchFinallyBlock: TryCatchFinallyBlock {
        return activeTryCatchFinallyBlock.top
    }

    private var activeLabels: [Label] = []

    public var hasLabel: Bool { activeLabels.count > 0 }

    public func randomLabel() -> Label {
        return chooseUniform(from: activeLabels)
    }
    ...
    public func reset() {
        ...
        activeTryCatchFinallyBlock.removeAll()
    }
    ...
    public func randomNumeric() -> Double {
        return chooseUniform(from: [randomFloat(), Double(randomInt())]);
    }
    ...
    public func randomCurrentClassDefinitionPrivateFieldName() -> String? {
        if currentClassDefinition.hasPrivateFields {
            return chooseUniform(from: currentClassDefinition.privateFields)
        } else if context.contains(.classDefinition) && !context.contains(.javascript) {
            let choosablePrivateFieldGenerators = ["ClassPrivateInstancePropertyGenerator", "ClassPrivateInstanceMethodGenerator", "ClassPrivateStaticPropertyGenerator", "ClassPrivateStaticMethodGenerator"]
            let choosenPrivateFieldGeneratorName = chooseUniform(from: choosablePrivateFieldGenerators)
            run(CodeGenerators.get(choosenPrivateFieldGeneratorName))
            return chooseUniform(from: currentClassDefinition.privateFields)
        } else {
            return nil
        }
    ...
    @discardableResult
    public func mysplice(_ program: Program, _ specifiedIndex: Int) -> Bool {
        let rootIndex = specifiedIndex 
        var requiredVariables = VariableSet()
        var outputVariables =  VariableSet()
        var index = rootIndex
        ...
        while index >= 0 {
            ...
            if includeCurrentInstruction {
                program.code.instructions[instr.index].is_target = 1
                let newlyRequiredVariables = instr.inputs
                requiredVariables.formUnion(newlyRequiredVariables)
            }
            index -= 1
        }

        let len = program.code.instructions.count
        index = rootIndex
        while index < len {
            let instr = program.code[index]
            var includeCurrentInstruction = false
            if index == rootIndex {
                includeCurrentInstruction = true
            } else if !outputVariables.isDisjoint(with: instr.inputs) {
                includeCurrentInstruction = true
            } 
            if includeCurrentInstruction {
                program.code.instructions[instr.index].is_target = 1
                let newlyOutputVariables = instr.allOutputs
                outputVariables.formUnion(newlyOutputVariables)
            }
            index += 1
        }
        return true
    ...
    public class ObjectLiteral {
        ...
        public fileprivate(set) var numericProperties: [Double] = []
        ...
        public fileprivate(set) var numericMethods: [Double] = []
        ...
        public fileprivate(set) var numericGetters: [Double] = []
        public fileprivate(set) var setters: [String] = []
        public fileprivate(set) var computedSetters: [Variable] = []
        public fileprivate(set) var numericSetters: [Double] = []
        ...
        public func addProperty(_ name: String, as value: Variable) {
            b.emit(ObjectLiteralAddStringProperty(propertyName: name), withInputs: [value])
        }

        public func addNumericProperty(_ name: Double, as value: Variable) {
            b.emit(ObjectLiteralAddNumericProperty(propertyName: name), withInputs: [value])
        }
        ...
        public func addMethod(_ name: String, isAsync asyncFlag: Bool = false, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginObjectLiteralStringMethod(methodName: name, isAsync: asyncFlag, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndObjectLiteralMethod())
        }

        public func addNumericMethod(_ name: Double, isAsync asyncFlag: Bool, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginObjectLiteralNumericMethod(methodName: name, isAsync: asyncFlag, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndObjectLiteralMethod())
        }

        public func addComputedMethod(_ name: Variable, isAsync asyncFlag: Bool, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginObjectLiteralComputedMethod(parameters: descriptor.parameters, isAsync: asyncFlag), withInputs: [name])
            body(Array(instr.innerOutputs))
            b.emit(EndObjectLiteralMethod())
        }

        public func addGetter(for name: String, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginObjectLiteralStringGetter(propertyName: name))
        ...
        public func addNumericGetter(for name: Double, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginObjectLiteralNumericGetter(propertyName: name))
            body(instr.innerOutput)
            b.emit(EndObjectLiteralGetter())
        }

        public func addSetter(for name: String, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginObjectLiteralStringSetter(propertyName: name))
        ...
        public func addNumericSetter(for name: Double, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginObjectLiteralNumericSetter(propertyName: name))
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndObjectLiteralSetter())
        }
    }
    ...
    public class ClassDefinition {
        ...
        public fileprivate(set) var instanceNumericMethods: [Double] = []
        public fileprivate(set) var instanceBigIntMethods: [Int64] = []
        public fileprivate(set) var instanceGetters: [String] = []
        public fileprivate(set) var instanceComputedGetters: [Variable] = []
        public fileprivate(set) var instanceNumericGetters: [Double] = []
        public fileprivate(set) var instanceSetters: [String] = []
        public fileprivate(set) var instanceComputedSetters: [Variable] = []
        public fileprivate(set) var instanceNumericSetters: [Double] = []
        ...
        public fileprivate(set) var staticNumericMethods: [Double] = []
        public fileprivate(set) var staticBigIntMethods: [Int64] = []
        public fileprivate(set) var staticGetters: [String] = []
        public fileprivate(set) var staticComputedGetters: [Variable] = []
        public fileprivate(set) var staticNumericGetters: [Double] = []
        public fileprivate(set) var staticSetters: [String] = []
        public fileprivate(set) var staticComputedSetters: [Variable] = []
        public fileprivate(set) var staticNumericSetters: [Double] = []
        ...
        public var hasPrivateFields: Bool {
            return privateFields.count > 0
        }
        ...
        public func addInstanceMethod(_ name: String, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassInstanceStringMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassInstanceMethod())
        }

        public func addInstanceComputedMethod(for name: Variable, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
        ...
        public func addInstanceNumericMethod(for name: Double, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassInstanceNumericMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassInstanceMethod())
        }

        public func addInstanceBigIntMethod(for name: Int64, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassInstanceBigIntMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassInstanceMethod())
        }

        public func addInstanceGetter(for name: String, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceStringGetter(propertyName: name))
            body(instr.innerOutput)
            b.emit(EndClassInstanceGetter())
        }

        public func addInstanceComputedGetter(for name: Variable, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceComputedGetter(), withInputs: [name])
            body(instr.innerOutput)
            b.emit(EndClassInstanceGetter())
        }

        public func addInstanceNumericGetter(for name: Double, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceNumericGetter(propertyName: name))
            body(instr.innerOutput)
            b.emit(EndClassInstanceGetter())
        }

        public func addInstanceSetter(for name: String, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceStringSetter(propertyName: name))
            body(instr.innerOutput(0), instr.innerOutput(1))
        ...
        public func addInstanceComputedSetter(for name: Variable, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceComputedSetter(), withInputs: [name])
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndClassInstanceSetter())
        }

        public func addInstanceNumericSetter(for name: Double, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassInstanceNumericSetter(propertyName: name))
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndClassInstanceSetter())
        }
        ...
        public func addStaticMethod(_ name: String, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassStaticStringMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassStaticMethod())
        }

        public func addStaticComputedMethod(for name: Variable, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
        ...
        public func addStaticNumericMethod(for name: Double, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassStaticNumericMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassStaticMethod())
        }

        public func addStaticBigIntMethod(for name: Int64, with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> ()) {
            b.setParameterTypesForNextSubroutine(descriptor.parameterTypes)
            let instr = b.emit(BeginClassStaticBigIntMethod(methodName: name, parameters: descriptor.parameters))
            body(Array(instr.innerOutputs))
            b.emit(EndClassStaticMethod())
        }

        public func addStaticGetter(for name: String, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticStringGetter(propertyName: name))
            body(instr.innerOutput)
            b.emit(EndClassStaticGetter())
        }

        public func addStaticComputedGetter(for name: Variable, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticComputedGetter(), withInputs: [name])
            body(instr.innerOutput)
            b.emit(EndClassStaticGetter())
        }

        public func addStaticNumericGetter(for name: Double, _ body: (_ this: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticNumericGetter(propertyName: name))
            body(instr.innerOutput)
            b.emit(EndClassStaticGetter())
        }

        public func addStaticSetter(for name: String, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticStringSetter(propertyName: name))
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndClassStaticSetter())
        }

        public func addStaticComputedSetter(for name: Variable, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticComputedSetter(), withInputs: [name])
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndClassStaticSetter())
        }

        public func addStaticNumericSetter(for name: Double, _ body: (_ this: Variable, _ val: Variable) -> ()) {
            let instr = b.emit(BeginClassStaticNumericSetter(propertyName: name))
            body(instr.innerOutput(0), instr.innerOutput(1))
            b.emit(EndClassStaticSetter())
        }
        ...
        public func addPrivateInstancePropertyWithPrivateInInitializer(_ prop: String, _ testInProp: String, _ obj: Variable? = nil) {
            let inputs = obj != nil ? [obj!]: []
            b.emit(ClassAddPrivateInstancePropertyWithPrivateInInitializer(propertyName: prop, testInPropertyName: testInProp, hasCustomInObject: obj != nil), withInputs: inputs)
        }
        ...
        public func addPrivateStaticPropertyWithPrivateInInitializer(_ prop: String, _ testInProp: String, _ obj: Variable? = nil) {
            let inputs = obj != nil ? [obj!]: []
            b.emit(ClassAddPrivateStaticPropertyWithPrivateInInitializer(propertyName: prop, testInPropertyName: testInProp, hasCustomInObject: obj != nil), withInputs: inputs)
        }
        ...
        @discardableResult
        public func addTestClassPrivateInInMethod(_ prop: String, _ obj: Variable) -> Variable {
            return b.emit(TestClassPrivateInInMethod(propertyName: prop), withInputs: [obj]).output
        }
    }
    ...
    /// Represents a currently active try-catch-finally-block. Used to add possible exception field to it
    /// and to query which fields already exist to use in destruct in catch.
    public class TryCatchFinallyBlock {
        private let b: ProgramBuilder

        public fileprivate(set) var possibleThrowExpressionProperties: [String] = []

        fileprivate init(in b: ProgramBuilder) {
            self.b = b
        }

        public var hasThrowExpressionProperty: Bool {
            return possibleThrowExpressionProperties.count > 0
        }

        public func addThrowExpressionProperty(_ name: String) {
            possibleThrowExpressionProperties.append(name)
        }
    }
    ...
    @discardableResult
    public func buildExpressionBodyArrowFunction(with descriptor: SubroutineDescriptor, _ body: ([Variable]) -> Variable?) -> Variable {
        setParameterTypesForNextSubroutine(descriptor.parameterTypes)
        let instr = emit(BeginExpressionBodyArrowFunction(parameters: descriptor.parameters, isStrict: false))
        if enableRecursionGuard { hide(instr.output) }
        var innerOutput: Variable
        if let bodyResult = body(Array(instr.innerOutputs)) {
            innerOutput = bodyResult
        } else {
            innerOutput = randomVariable()
        }
        if enableRecursionGuard { unhide(instr.output) }
        emit(EndExpressionBodyArrowFunction(), withInputs: [innerOutput])
        return instr.output
    }
    ...
    public func jumpToLabel(jumpType: JumpType, label: Label) {
        emit(JumpToLabel(jumpType: jumpType, label: label))
    }
    ...
    public func buildForOfLoop(_ obj: Variable, _ isAsync: Bool, _ body: (Variable) -> ()) {
        let i = emit(BeginForOfLoop(isAsync: isAsync), withInputs: [obj]).innerOutput
        body(i)
        emit(EndForOfLoop())
    }
    ...
    public func buildTryCatchWithPossibleDestructFinally(tryBody: () -> (), catchBody: (([Variable]) -> ())? = nil, finallyBody: (() -> ())? = nil, enforceDestruct: Bool = false) {
        assert(catchBody != nil || finallyBody != nil, "Must have either a Catch or a Finally block (or both)")
        emit(BeginTry())
        tryBody()
        if let catchBody = catchBody {
            let operation: Operation
            if currentTryCatchFinallyBlock.hasThrowExpressionProperty {
                operation = BeginCatchWithDestruct(destructPropertyNames: currentTryCatchFinallyBlock.possibleThrowExpressionProperties)
            } else {
                operation = BeginCatch()
            }
            let exception = Array(emit(operation).innerOutputs)
            catchBody(exception)
        }
        if let finallyBody = finallyBody {
            emit(BeginFinally())
    ...
    private func updateBuilderState(_ instr: Instruction) {
        ...
        case .objectLiteralAddNumericProperty(let op):
            currentObjectLiteral.numericProperties.append(op.propertyName)
        ...
        case .beginObjectLiteralNumericMethod(let op):
            currentObjectLiteral.numericMethods.append(op.methodName)
        ...
        case .beginObjectLiteralNumericGetter(let op):
            currentObjectLiteral.numericGetters.append(op.propertyName)
        ...
        case .beginObjectLiteralNumericSetter(let op):
            currentObjectLiteral.numericSetters.append(op.propertyName)
        ...
        case .beginClassInstanceNumericMethod(let op):
            activeClassDefinitions.top.instanceNumericMethods.append(op.methodName)
        case .beginClassInstanceBigIntMethod(let op):
            activeClassDefinitions.top.instanceBigIntMethods.append(op.methodName)
        ...
        case .beginClassInstanceNumericGetter(let op):
            activeClassDefinitions.top.instanceNumericGetters.append(op.propertyName)
        ...
        case .beginClassInstanceNumericSetter(let op):
            activeClassDefinitions.top.instanceNumericSetters.append(op.propertyName)
        ...
        case .beginClassStaticNumericMethod(let op):
            activeClassDefinitions.top.staticNumericMethods.append(op.methodName)
        case .beginClassStaticBigIntMethod(let op):
            activeClassDefinitions.top.staticBigIntMethods.append(op.methodName)
        ...
        case .beginClassStaticNumericGetter(let op):
            activeClassDefinitions.top.staticNumericGetters.append(op.propertyName)
        ...
        case .beginClassStaticNumericSetter(let op):
            activeClassDefinitions.top.staticNumericSetters.append(op.propertyName)
        ...
        case .testClassPrivateInInMethod:
            break
        ...
        case .beginTry:
            activeTryCatchFinallyBlock.push(TryCatchFinallyBlock(in: self))
        case .throwException:
            if context.contains(.tryBlock) {
                let expressionType = type(of: instr.input(0))
                for property in expressionType.properties {
                    activeTryCatchFinallyBlock.top.addThrowExpressionProperty(property)
                }
            }
        case .endTryCatchFinally:
            activeTryCatchFinallyBlock.pop()

        case .jumpToLabel(let op):
            assert(op.label.isHolderAlive())
            assert(op.label.bindable(op))
            op.label.addReferencer(op)
