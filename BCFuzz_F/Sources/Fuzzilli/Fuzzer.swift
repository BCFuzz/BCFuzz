...
public class Fuzzer {
    ...
    // wjm: num of testcases mutated from bc
    public var bc_testcase_num: Double = 0.0

    // wjm: num of testcases preserved old bc 
    public var bc_preserve_num: Double = 0.0

    // wjm: num of testcases preserved old bc and have new edges
    public var bc_new_edge: Double = 0.0

    /// The engine used for initial corpus generation (if performed).
    public let corpusGenerationEngine = GenerativeEngine()

    //public var myfuzzing = 0
    ...
    public func foundIR() -> Int {
        guard let covEvaluator = evaluator as? ProgramCoverageEvaluator else {
            logger.fatal("Fuzzer needs to be provided a CoverageEvaluator")
        }
        return covEvaluator.foundIR
    }
    ...
    public let fuzzGroup = DispatchGroup()
    ...
    public func initialize() {
        ...
        /*
        timers.scheduleTask(every: 10 * Minutes) {
            self.myfuzzing = 1
        }
        */
        ...
        timers.scheduleTask(every: 72 * 60 * Minutes) {
            self.dispatchEvent(self.events.Shutdown, data: .finished)
        }
    ...
    public func importProgram(_ program: Program, enableDropout: Bool = false, origin: ProgramOrigin) -> ExecutionOutcome {
        ...
        case .succeeded:
            var imported = false
            if let aspects = evaluator.evaluate(execution, program) {
    ...
    private func fuzzOne() {
        ...
        case .corpusGeneration:
            ...
            if iterationsSinceLastInterestingProgram > 100 /*|| myfuzzing == 1*/ {
                guard !corpus.isEmpty else {
