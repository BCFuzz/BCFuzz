...
public class JavaScriptLifter: Lifter {
    ...
    public func lift(_ program: Program, withOptions options: LiftingOptions) -> String {
        ...
        var ii = 0;
        w.emit("//empty line") // distinguish between the builtin line and the real line
        for instr in program.code {
            //w.emit("/* instr */")
            ...
            case .objectLiteralAddNumericProperty(let op):
                ...
                let PROPERTY = op.propertyName < 0 ? "[\(op.propertyName)]" : String(op.propertyName)
                let VALUE = input(0)
                w.emit("\(PROPERTY): \(VALUE),")
            ...
            case .beginObjectLiteralStringMethod(let op):
                ...
                let asyncTag = op.isAsync ? "*" : ""
                let METHOD = op.methodName
                w.emit("\(asyncTag)\(METHOD)(\(PARAMS)) {")
                w.enterNewBlock()
            ...
            case .beginObjectLiteralNumericMethod(let op):
                ...
                let asyncTag = op.isAsync ? "*" : ""
                let METHOD = op.methodName
                var methodNamePlaceholder = METHOD.truncatingRemainder(dividingBy: 1.0) == 0 ? "%.0f" : "%f"
                methodNamePlaceholder = METHOD < 0 ? "[\(methodNamePlaceholder)]" : methodNamePlaceholder
                let METHOD_STRING = String(format: methodNamePlaceholder, METHOD)
                w.emit("\(asyncTag)\(METHOD_STRING)(\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginObjectLiteralNumericGetter(let op):
                assert(instr.numInnerOutputs == 1)
                let PROPERTY = op.propertyName
                var propertyNamePlaceholder = PROPERTY.truncatingRemainder(dividingBy: 1.0) == 0 ? "%.0f" : "%f"
                propertyNamePlaceholder = PROPERTY < 0 ? "[\(propertyNamePlaceholder)]" : propertyNamePlaceholder
                let PROPERTY_STRING = String(format: propertyNamePlaceholder, PROPERTY)
                w.emit("get \(PROPERTY_STRING)() {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginObjectLiteralNumericSetter(let op):
                ...
                propertyNamePlaceholder = PROPERTY < 0 ? "[\(propertyNamePlaceholder)]" : propertyNamePlaceholder
                let PROPERTY_STRING = String(format: propertyNamePlaceholder, PROPERTY)
                w.emit("set \(PROPERTY_STRING)(\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginClassInstanceBigIntMethod(let op):
                let vars = w.declareAll(instr.innerOutputs.dropFirst(), usePrefix: "a")
                let PARAMS = liftParameters(op.parameters, as: vars)
                let METHOD = op.methodName < 0 ? "[\(op.methodName)n]" : "\(op.methodName)n"
                w.emit("\(METHOD)(\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginClassInstanceComputedSetter(let op):
                assert(instr.numInnerOutputs == 2)
                let vars = w.declareAll(instr.innerOutputs.dropFirst(), usePrefix: "a")
                let PARAMS = liftParameters(op.parameters, as: vars)
                let METHOD = input(0)
            ...
            case .beginClassStaticNumericMethod(let op):
                ...
                methodNamePlaceholder = METHOD < 0 ? "[\(methodNamePlaceholder)]" : methodNamePlaceholder
                let METHOD_STRING = String(format: methodNamePlaceholder, METHOD)
                w.emit("static \(METHOD_STRING)(\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginClassStaticNumericGetter(let op):
                ...
                propertyNamePlaceholder = PROPERTY < 0 ? "[\(propertyNamePlaceholder)]" : propertyNamePlaceholder
                let PROPERTY_STRING = String(format: propertyNamePlaceholder, PROPERTY)
                w.emit("static get \(PROPERTY_STRING)() {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput)
            ...
            case .beginClassStaticComputedSetter(let op):
                assert(instr.numInnerOutputs == 2)
                let vars = w.declareAll(instr.innerOutputs.dropFirst(), usePrefix: "a")
                let PARAMS = liftParameters(op.parameters, as: vars)
                let METHOD = input(0)
                w.emit("static set [\(METHOD)](\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .beginClassStaticNumericSetter(let op):
                ...
                propertyNamePlaceholder = PROPERTY < 0 ? "[\(propertyNamePlaceholder)]" : propertyNamePlaceholder
                let PROPERTY_STRING = String(format: propertyNamePlaceholder, PROPERTY)
                w.emit("static set \(PROPERTY_STRING)(\(PARAMS)) {")
                w.enterNewBlock()
                bindVariableToThis(instr.innerOutput(0))
            ...
            case .classAddPrivateInstancePropertyWithPrivateInInitializer(let op):
                let PROPERTY = op.propertyName
                let TEST_IN_PROPERTY = op.testInPropertyName
                if op.hasCustomInObject {
                    let CUSTOM_IN_OBJECT = input(0)
                    w.emit("#\(PROPERTY) = #\(TEST_IN_PROPERTY) in \(CUSTOM_IN_OBJECT);")
                } else {
                    w.emit("#\(PROPERTY) = #\(TEST_IN_PROPERTY) in this;")
                }
            ...
            case .classAddPrivateStaticPropertyWithPrivateInInitializer(let op):
                ...
                if op.hasCustomInObject {
                    let CUSTOM_IN_OBJECT = input(0)
                    w.emit("static #\(PROPERTY) = #\(TEST_IN_PROPERTY) in \(CUSTOM_IN_OBJECT);")
                } else {
                    w.emit("static #\(PROPERTY) = #\(TEST_IN_PROPERTY) in this;")
                }
            ...
            case .beginArrowFunction(let op):
                ...
                let PARAMS = liftParameters(op.parameters, as: vars)
                let paramParenthesis =
                    !op.parameters.hasRestParameter && op.parameters.count == 1
                    ? ["", ""] : ["(", ")"]
                w.emit("\(LET) \(V) = \(paramParenthesis[0])\(PARAMS)\(paramParenthesis[1]) => {")
                w.enterNewBlock()
                if op.isStrict {
                    w.emit("'use strict';")
                }
            ...
            case .endExpressionBodyArrowFunction:
                w.emitBlock(handleEndSingleExpressionContext(result: input(0), with: &w))
                w.leaveCurrentBlock()
                w.emit(";")
            ...
            case .beginForOfLoop(let op):
                ...
                let LET = w.declarationKeyword(for: instr.innerOutput)
                let OBJ = input(0)
                w.emit("for \(isAsync)(\(LET) \(V) of \(OBJ)) {")
                w.enterNewBlock()
            ...
            case .beginCatchWithDestruct(let op):
                w.declareAll(instr.innerOutputs, as: op.destructPropertyNames)
                w.leaveCurrentBlock()
                let E = "{\(liftCatchDestructPattern(javascriptIdentifiers: op.destructPropertyNames))}"
                w.emit("} catch( \(E) ) {")
                w.enterNewBlock()
            ...
            // wirte instr will not update program.code.instructions[ii]
            // so we update program.code.instructions[ii] directly
            program.code.instructions[ii].line = w.get_line()
            ii = ii + 1
        }

        //for instr in program.code {
        //    print("FuzzIL line: \(instr.line)")
        //}
    ...
    private func liftCatchDestructPattern(javascriptIdentifiers: [String]) -> String {
        var catchPattern = ""
        for javascriptIdentifier in javascriptIdentifiers {
            catchPattern += "\(javascriptIdentifier), "
        }
        return catchPattern
    }
    ...
    private struct JavaScriptWriter {
        ...
        public func get_line() -> Int {
            return writer.get_line()
        }
