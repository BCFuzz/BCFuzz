...
public let CodeGenerators: [CodeGenerator] = [
    ...
    CodeGenerator("ObjectLiteralNumericPropertyGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))
        ...
        repeat {
            guard attempts < 10 else { return }
            propertyName = b.randomFloat()
            attempts += 1
        } while b.currentObjectLiteral.numericProperties.contains(propertyName)

        b.currentObjectLiteral.addNumericProperty(propertyName, as: b.randomVariable())
    },
    ...
    RecursiveCodeGenerator("ObjectLiteralMethodGenerator", inContext: .objectLiteral) { b in
        ...
        b.currentObjectLiteral.addMethod(methodName, isAsync: probability(0.2), with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ObjectLiteralNumericMethodGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))

        // Try to find a numeric method name that hasn't already been added to this literal.
        var methodName: Double
        var attempts = 0
        repeat {
            guard attempts < 10 else { return }
            methodName = b.randomNumeric()
            attempts += 1
        } while b.currentObjectLiteral.numericMethods.contains(methodName)

        var isAsync = probability(0.2)

        b.currentObjectLiteral.addNumericMethod(methodName, isAsync: isAsync, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ObjectLiteralComputedMethodGenerator", inContext: .objectLiteral) { b in
        ...
        b.currentObjectLiteral.addComputedMethod(methodName, isAsync: isAsync, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ObjectLiteralComputedGetterGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))

        // Try to find a computed getter name that hasn't already been added and for which a getter has not yet been installed.
        var propertyName: Variable
        var attempts = 0
        repeat {
            guard attempts < 10 else { return }
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentObjectLiteral.computedMethods.contains(propertyName) || b.currentObjectLiteral.computedGetters.contains(propertyName)

        b.currentObjectLiteral.addComputedGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ObjectLiteralNumericGetterGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))
        ...
        repeat {
            guard attempts < 10 else { return }
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentObjectLiteral.numericGetters.contains(propertyName)

        b.currentObjectLiteral.addNumericGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ObjectLiteralComputedSetterGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))

        // Try to find a computed setter name that hasn't already been added and for which a setter has not yet been installed.
        var propertyName: Variable
        var attempts = 0
        repeat {
            guard attempts < 10 else { return }
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentObjectLiteral.computedMethods.contains(propertyName) || b.currentObjectLiteral.computedSetters.contains(propertyName)

        b.currentObjectLiteral.addComputedSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },

    RecursiveCodeGenerator("ObjectLiteralNumericSetterGenerator", inContext: .objectLiteral) { b in
        assert(b.context.contains(.objectLiteral) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentObjectLiteral.numericSetters.contains(propertyName)

        b.currentObjectLiteral.addNumericSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },
    ...
    RecursiveCodeGenerator("ClassInstanceComputedMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            methodName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.instanceComputedMethods.contains(methodName)

        b.currentClassDefinition.addInstanceComputedMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassInstanceNumericMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            methodName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.instanceNumericMethods.contains(methodName)

        b.currentClassDefinition.addInstanceNumericMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassInstanceBigIntMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))

        // Try to find a method that hasn't already been added to this class.
        var methodName: Int64
        var attempts = 0
        repeat {
            guard attempts < 10 else { return }
            methodName = b.randomInt64()
            attempts += 1
        } while b.currentClassDefinition.instanceBigIntMethods.contains(methodName)

        b.currentClassDefinition.addInstanceBigIntMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ClassInstanceComputedGetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.instanceComputedProperties.contains(propertyName) || b.currentClassDefinition.instanceComputedGetters.contains(propertyName)

        b.currentClassDefinition.addInstanceComputedGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassInstanceNumericGetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.instanceNumericGetters.contains(propertyName)

        b.currentClassDefinition.addInstanceNumericGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ClassInstanceComputedSetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.instanceComputedProperties.contains(propertyName) || b.currentClassDefinition.instanceComputedSetters.contains(propertyName)

        b.currentClassDefinition.addInstanceComputedSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },

    RecursiveCodeGenerator("ClassInstanceNumericSetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.instanceNumericSetters.contains(propertyName)

        b.currentClassDefinition.addInstanceNumericSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },
    ...
    RecursiveCodeGenerator("ClassStaticComputedMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            methodName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.staticComputedMethods.contains(methodName)

        b.currentClassDefinition.addStaticComputedMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassStaticNumericMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            methodName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.staticNumericMethods.contains(methodName)

        b.currentClassDefinition.addStaticNumericMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassStaticBigIntMethodGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            methodName = b.randomInt64()
            attempts += 1
        } while b.currentClassDefinition.staticBigIntMethods.contains(methodName)

        b.currentClassDefinition.addStaticBigIntMethod(for: methodName, with: b.randomParameters()) { args in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ClassStaticComputedGetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.staticComputedProperties.contains(propertyName) || b.currentClassDefinition.staticComputedGetters.contains(propertyName)

        b.currentClassDefinition.addStaticComputedGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },

    RecursiveCodeGenerator("ClassStaticNumericGetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.staticNumericGetters.contains(propertyName)

        b.currentClassDefinition.addStaticNumericGetter(for: propertyName) { this in
            b.buildRecursive()
            b.doReturn(b.randomVariable())
        }
    },
    ...
    RecursiveCodeGenerator("ClassStaticComputedSetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomVariable()
            attempts += 1
        } while b.currentClassDefinition.staticComputedProperties.contains(propertyName) || b.currentClassDefinition.staticComputedSetters.contains(propertyName)

        b.currentClassDefinition.addStaticComputedSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },

    RecursiveCodeGenerator("ClassStaticNumericSetterGenerator", inContext: .classDefinition) { b in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        repeat {
            ...
            propertyName = b.randomNumeric()
            attempts += 1
        } while b.currentClassDefinition.staticNumericSetters.contains(propertyName)

        b.currentClassDefinition.addStaticNumericSetter(for: propertyName) { this, v in
            b.buildRecursive()
        }
    },
    ...
    RecursiveCodeGenerator("ClassPrivateInstancePropertyWithPrivateInInitializerGenerator", inContext: .classDefinition, inputs: .preferred(.object())) { b, obj in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))

        if let privateFieldName = b.randomCurrentClassDefinitionPrivateFieldName() {
            // Try to find a private field that hasn't already been added to this literal.
            var newPrivateFieldName: String
            var attempts = 0
            repeat {
                guard attempts < 10 else { return }
                newPrivateFieldName = b.randomCustomPropertyName()
                attempts += 1
            } while b.currentClassDefinition.privateFields.contains(newPrivateFieldName)

            var customInObject: Variable? = probability(0.5) ? obj : nil
            b.currentClassDefinition.addPrivateInstancePropertyWithPrivateInInitializer(newPrivateFieldName, privateFieldName, customInObject)
        }
    },
    ...
    RecursiveCodeGenerator("ClassPrivateStaticPropertyWithPrivateInInitializerGenerator", inContext: .classDefinition, inputs: .preferred(.object())) { b, obj in
        assert(b.context.contains(.classDefinition) && !b.context.contains(.javascript))
        ...
        if let privateFieldName = b.randomCurrentClassDefinitionPrivateFieldName() {
            ...
            var customInObject: Variable? = probability(0.5) ? obj : nil
            b.currentClassDefinition.addPrivateStaticPropertyWithPrivateInInitializer(newPrivateFieldName, privateFieldName, customInObject)
        }
    },
    ...
    RecursiveCodeGenerator("ArrowFunctionGenerator") { b in
        if probability(0.5) {
            ...
            b.buildExpressionBodyArrowFunction(with: b.randomParameters()) { _ in
                b.buildRecursive()
    ...
    CodeGenerator("ClassPrivateInInMethodGenerator", inContext: .classMethod, inputs: .preferred(.object())) { b, obj in
        assert(b.context.contains(.classMethod) && b.context.contains(.javascript))

        if let privateFieldName = b.randomCurrentClassDefinitionPrivateFieldName() {
            b.currentClassDefinition.addTestClassPrivateInInMethod(privateFieldName, obj)
        }
    },
    ...
    RecursiveCodeGenerator("ForOfLoopGenerator", inputs: .preferred(.iterable)) { b, obj in
        b.buildForOfLoop(obj, probability(0.5)) { _ in
            b.buildRecursive()
        }
    },
    ...
    RecursiveCodeGenerator("TryCatchGenerator") { b in
        ...
        withEqualProbability({
            // try-catch-finally
            b.buildTryCatchWithPossibleDestructFinally(tryBody: {
        ...
        if b.type(of: orig).Is(.primitive) {
            ...
            if probability(0.5) {
                imitation = b.buildObjectLiteral { obj in
                    obj.addMethod("valueOf", isAsync: false, with: .parameters(n: 0)) { _ in
                ...
                imitation = b.buildObjectLiteral { obj in
                    obj.addComputedMethod(toPrimitive, isAsync: false, with: .parameters(n: 0)) { _ in
                        b.buildRecursive(n: 3)
                        b.doReturn(orig)
                    }
                }
    ...
    CodeGenerator("IteratorGenerator") { b in
        ...
        let iterableObject = b.buildObjectLiteral { obj in
            obj.addComputedMethod(iteratorSymbol, isAsync: false, with: .parameters(n: 0)) { _ in
                let counter = b.loadInt(10)
                let iterator = b.buildObjectLiteral { obj in
                    obj.addMethod("next", isAsync: false, with: .parameters(n: 0)) { _ in
    ...
    CodeGenerator("JumpToLabelGenerator", inContext: .javascript) { b in
        assert(b.context.contains(.javascript))
        guard b.hasLabel else { return }

        let label = b.randomLabel()

        b.jumpToLabel(jumpType: label.randomAssociatableJumpType(), label: label)
    },
