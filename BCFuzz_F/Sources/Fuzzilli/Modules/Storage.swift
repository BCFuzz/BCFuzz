...
public class Storage: Module {
    ...
    public init(for fuzzer: Fuzzer, storageDir: String, statisticsExportInterval: Double? = nil) {
        ...
        self.corpusDir2 = storageDir + "/ir_corpus"
    ...
    public func initialize(with fuzzer: Fuzzer) {
        do {
            ...
            try FileManager.default.createDirectory(atPath: corpusDir2, withIntermediateDirectories: true)
        ...
        fuzzer.registerEventListener(for: fuzzer.events.NewIRProgramFound) { ev in
            let filename = "program_\(self.formatDate())_\(ev.id)"
            self.storeProgram(ev, as: filename, in: self.corpusDir2)
        }
    ...
    private func storeProgram(_ program: Program, as filename: String, in directory: String) {
        ...
        //let options = LiftingOptions.includeLineNumbers
        let code = fuzzer.lifter.lift(program, withOptions: options)
        var mycontent = code

        //mycontent += "\n//*******test begin************\n"
        //for instr in program.code {
        //    mycontent += "//    FuzzIL line: \(instr.line)\n"
        //}
        //mycontent += "\n//*******test begin************\n"
        
        //mycontent += "\n // program interesting FuzzIL: \(program.interesting_index) \n"

        mycontent += "\n// ============== FuzzIL ==============\n"
        mycontent += "/*\n"
        mycontent += "\(FuzzILLifter().lift(program))"
        mycontent += "*/\n\n"

        let url = URL(fileURLWithPath: "\(directory)/\(filename).js")
        createFile(url, withContent: mycontent)
