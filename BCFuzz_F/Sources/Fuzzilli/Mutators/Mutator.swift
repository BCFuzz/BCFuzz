...
public class Mutator: Contributor {
    ...
    public final func mutate2(_ program: Program, for fuzzer: Fuzzer) -> Program? {
        let b = fuzzer.makeBuilder(forMutating: program)
        b.traceHeader("Bytecode Mutating \(program.id) with \(name)")
        let program = mutate2(program, using: b, for: fuzzer)
        program?.contributors.insert(self)
        return program
    }
    ...
    func mutate2(_ program: Program, using b: ProgramBuilder, for fuzzer: Fuzzer) -> Program? {
        fatalError("This method must be overridden")
    }
