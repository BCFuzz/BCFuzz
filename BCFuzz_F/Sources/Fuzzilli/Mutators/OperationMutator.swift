...
public class OperationMutator: BaseInstructionMutator {
    ...
    private func mutateOperation(_ instr: Instruction, _ b: ProgramBuilder) -> Instruction {
        ...
        case .objectLiteralAddStringProperty:
            newOp = ObjectLiteralAddStringProperty(propertyName: b.randomPropertyName())
        case .objectLiteralAddNumericProperty:
            newOp = ObjectLiteralAddNumericProperty(propertyName: b.randomNumeric())
        case .objectLiteralAddElement:
            newOp = ObjectLiteralAddElement(index: b.randomIndex())
        case .beginObjectLiteralStringMethod(let op):
            newOp = BeginObjectLiteralStringMethod(methodName: b.randomMethodName(), isAsync: probability(0.2), parameters: op.parameters)
        case .beginObjectLiteralNumericMethod(let op):
            newOp = BeginObjectLiteralNumericMethod(methodName: b.randomNumeric(), isAsync: probability(0.2), parameters: op.parameters)
        case .beginObjectLiteralStringGetter:
            newOp = BeginObjectLiteralStringGetter(propertyName: b.randomPropertyName())
        case .beginObjectLiteralNumericGetter:
            newOp = BeginObjectLiteralNumericGetter(propertyName: b.randomNumeric())
        case .beginObjectLiteralStringSetter:
            newOp = BeginObjectLiteralStringSetter(propertyName: b.randomPropertyName())
        case .beginObjectLiteralNumericSetter:
            newOp = BeginObjectLiteralNumericSetter(propertyName: b.randomNumeric())
        ...
        case .beginClassInstanceStringMethod(let op):
            newOp = BeginClassInstanceStringMethod(methodName: b.randomMethodName(), parameters: op.parameters)
        case .beginClassInstanceNumericMethod(let op):
            newOp = BeginClassInstanceNumericMethod(methodName: b.randomNumeric(), parameters: op.parameters)
        case .beginClassInstanceBigIntMethod(let op):
            newOp = BeginClassInstanceBigIntMethod(methodName: b.randomInt64(), parameters: op.parameters)
        case .beginClassInstanceStringGetter:
            newOp = BeginClassInstanceStringGetter(propertyName: b.randomPropertyName())
        case .beginClassInstanceNumericGetter:
            newOp = BeginClassInstanceNumericGetter(propertyName: b.randomNumeric())
        case .beginClassInstanceStringSetter:
            newOp = BeginClassInstanceStringSetter(propertyName: b.randomPropertyName())
        case .beginClassInstanceNumericSetter:
            newOp = BeginClassInstanceNumericSetter(propertyName: b.randomNumeric())
        ...
        case .beginClassStaticStringMethod(let op):
            newOp = BeginClassStaticStringMethod(methodName: b.randomMethodName(), parameters: op.parameters)
        case .beginClassStaticNumericMethod(let op):
            newOp = BeginClassStaticNumericMethod(methodName: b.randomNumeric(), parameters: op.parameters)
        case .beginClassStaticBigIntMethod(let op):
            newOp = BeginClassStaticBigIntMethod(methodName: b.randomInt64(), parameters: op.parameters)
        case .beginClassStaticStringGetter:
            newOp = BeginClassStaticStringGetter(propertyName: b.randomPropertyName())
        case .beginClassStaticNumericGetter:
            newOp = BeginClassStaticNumericGetter(propertyName: b.randomNumeric())
        case .beginClassStaticStringSetter:
            newOp = BeginClassStaticStringSetter(propertyName: b.randomPropertyName())
        case .beginClassStaticNumericSetter:
            newOp = BeginClassStaticNumericSetter(propertyName: b.randomNumeric())
        case .createIntArray:
            var values = [Int64]()
        ...
        case .jumpToLabel(let op):
            op.destroy()
            let newLabel = b.randomLabel()
            newOp = JumpToLabel(jumpType: newLabel.randomAssociatableJumpType(), label: newLabel)
        default:
