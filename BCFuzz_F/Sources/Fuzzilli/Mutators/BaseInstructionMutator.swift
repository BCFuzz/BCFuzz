...
public class BaseInstructionMutator: Mutator {
    ...
    override final func mutate2(_ program: Program, using b: ProgramBuilder, for fuzzer: Fuzzer) -> Program? {
        beginMutation(of: program)

        var candidates = [Int]()
        for instr in program.code {
            if canMutate(instr) { // canMutate is related to beginMutation in CodeGenMutator
                if instr.is_target == 1 {
                    candidates.append(instr.index)
                    candidates.append(instr.index)
                }
                candidates.append(instr.index)
        ...
        */
        ...
        //print("bytecode mutate candidate size: \(candidates.count)")
