...
class LifterTests: XCTestCase {
    ...
    func testObjectLiteralLifting() {
        ...
        b.buildObjectLiteral { obj in
            ...
            obj.addNumericProperty(1.1, as: v2)
            obj.addNumericProperty(-1.1, as: v2)
            ...
            obj.addMethod("m2", isAsync: true, with: .parameters(n: 2)) { args in
                let r = b.binary(args[1], args[2], with: .Sub)
                b.doReturn(r)
            }
            obj.addNumericMethod(2.2, isAsync: true, with: .parameters(n: 0)) { args in
                b.doReturn(v1)
            }
            obj.addComputedMethod(toPrimitive, isAsync: false, with: .parameters(n: 0)) { args in
                b.doReturn(v1)
            }
            obj.addComputedMethod(toPrimitive, isAsync: true, with: .parameters(n: 0)) { args in
                b.doReturn(v1)
            }
            ...
            obj.addComputedGetter(for: v1) { this in
                let r = b.getProperty("p", of: this)
                b.doReturn(r)
            }
            obj.addNumericGetter(for: 1.1) { this in
                let r = b.getProperty("p", of: this)
                b.doReturn(r)
            }
            ...
            obj.addComputedSetter(for: v1) { this, v in
                b.setProperty("p", of: this, to: v)
            }
            obj.addNumericSetter(for: 2) { this, v in
                b.setProperty("p", of: this, to: v)
            }
    ...
    func testClassDefinitionLifting() {
        ...
        let C = b.buildClassDefinition() { cls in
            ...
            cls.addInstanceComputedMethod(for: baz42, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            cls.addInstanceNumericMethod(for: -1.1, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            cls.addInstanceBigIntMethod(for: -1, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            ...
            cls.addInstanceComputedGetter(for: i) { this in
                b.doReturn(b.loadInt(1337))
            }
            cls.addInstanceNumericGetter(for: 1.1) { this in
                b.doReturn(b.loadInt(1337))
            }
            cls.addInstanceSetter(for: "baz") { this, v in
            }
            cls.addInstanceComputedSetter(for: i) { this, v in
            }
            cls.addInstanceNumericSetter(for: 2) { this, v in
            }
            ...
            cls.addStaticComputedMethod(for: baz42, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            cls.addStaticNumericMethod(for: 1.1, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            cls.addStaticBigIntMethod(for: 1, with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = b.getProperty("foo", of: this)
                b.doReturn(foo)
            }
            ...
            cls.addStaticComputedGetter(for: two) { this in
                b.doReturn(b.loadInt(1337))
            }
            cls.addStaticNumericGetter(for: 2.2) { this in
                b.doReturn(b.loadInt(1337))
            }
            cls.addStaticSetter(for: "baz") { this, v in
            }
            cls.addStaticComputedSetter(for: two) { this, v in
            }
            cls.addStaticNumericSetter(for: -3) { this, val in
            }
            ...
            cls.addPrivateStaticPropertyWithPrivateInInitializer("privateInStatic", "sfoo")
            cls.addPrivateInstancePropertyWithPrivateInInitializer("privateInInstance", "ifoo")
            cls.addInstanceMethod("privateInInstanceMethod", with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = cls.addTestClassPrivateInInMethod("ifoo", this)
                b.doReturn(foo)
            }
            cls.addStaticMethod("privateInStaticMethod", with: .parameters(n: 0)) { params in
                let this = params[0]
                let foo = cls.addTestClassPrivateInInMethod("sfoo", this)
                b.doReturn(foo)
            }
        }
        ...
        class C4 {
            ...
            static #privateInStatic = #sfoo in this;
            #privateInInstance = #ifoo in this;
            privateInInstanceMethod() {
                return #ifoo in this;
            }
            static privateInStaticMethod() {
                return #sfoo in this;
            }
        }
        new C4(42);
    ...
    func testBinaryOperationLifting() {
        ...
        let nullishCoalesce = b.binary(undefined, v, with: .NilCoals)
        b.callFunction(print, withArgs: [nullishCoalesce])
    ...
    func testFunctionLifting() {
        ...
        b.reassign(f, to: f2)
        b.callFunction(f)
        let f3 = b.buildArrowFunction(with: .parameters(n: 1)) { args in
            b.doReturn(args[0])
        }
        b.callFunction(f3, withArgs: [b.loadFloat(10.37)]);
        let f4 = b.buildExpressionBodyArrowFunction(with: .parameters(n: 1)) { args in
            let x = b.binary(args[0], b.loadFloat(1.1), with: .Sub)
            let y = b.binary(args[0], b.loadInt(16), with: .Add)
            let z = b.binary(x, y, with: .Mul)
            return z
        }
        b.callFunction(f4, withArgs: [b.loadFloat(15.21)]);
    ...
    func testTryCatchWithDestructLifting() {
        let fuzzer = makeMockFuzzer()
        let b = fuzzer.makeBuilder()
        ...
        let f = b.buildPlainFunction(with: .parameters(n: 2)) { args in
            b.buildTryCatchWithPossibleDestructFinally(tryBody: {
                let v = b.binary(args[0], args[1], with: .Mul)
                let e1 = b.buildObjectLiteral { obj in
                    obj.addProperty("x", as: v)
                }
                let e2 = b.buildObjectLiteral { obj in
    ...
    func testForLoopWithArrayDestructLifting() {
        ...
        b.buildForOfLoop(a4, selecting: [0,2], hasRestElement: true) { args in
            ...
            b.buildForOfLoop(args[1], false) { v in
                b.callFunction(print, withArgs: [v])
            }
            b.buildForOfLoop(args[1], true) { v in
                b.callFunction(print, withArgs: [v])
            }
        }
    ...
    func testVoidOperator() {
        let fuzzer = makeMockFuzzer()
        let b = fuzzer.makeBuilder()

        let v0 = b.loadInt(0)
        let v1 = b.loadFloat(2.2)
        let v2 = b.binary(v0, v1, with: .Add)
        let v3 = b.unary(.PreVoid, v2)
    ...
    func testJumpToLabelLifting() {
        let fuzzer = makeMockFuzzer()
        let b = fuzzer.makeBuilder()

        b.blockStatement {
            let label = b.randomLabel()
            b.jumpToLabel(jumpType: JumpType.Break, label: label)
        }
        b.buildRepeatLoop(n: 2) {
            let label = b.randomLabel()
            b.jumpToLabel(jumpType: JumpType.Continue, label: label)
        }
