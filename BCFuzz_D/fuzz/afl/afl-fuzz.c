...
EXP_ST u8 *in_dir,                    /* Input directory with test cases  */
          ...
          *sync_id,                   /* Fuzzer ID                        */
          *fuzzer_id,                 /* Fuzzer ID for reporting          */
...
EXP_ST u8* trace_bits2;                /* SHM with instrumentation bitmap  */

EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           virgin_tmout[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
           virgin_crash[MAP_SIZE];    /* Bits we haven't seen in crashes  */

EXP_ST u8 virgin_bits2[MY_SIZE];
EXP_ST int bc_test_bits[MY_SIZE];     /* Record test nums of different bcs*/

EXP_ST int tmp_bc_test[MY_SIZE]; /*Record non zero test nums of different bcs*/
static int tmp_bc_num = 0;

EXP_ST u32 less_bc_id[MY_SIZE]; // record bc id that currently test less
static int less_bc_num = 0;     // less_bc_num < 4096
static int bc_cursor=0;         // iterate less_bc_id, bc_cursor < less_bc_num

static u32 found_bc = 0;    // new bc num
static int current_bc = -1; // current tested bytecode id, change when a bc bytecode is selected

static u32 new_bits_num = 0;
static u32 new_bits[2 * MAP_SIZE];
...
#define JS_MUTATE_NUM 100
#define JS_INSERT_NUM 50
...
static int execute_sh(char* cmd) {
  ACTF("Command: %s", cmd);
  return system(cmd);
}
...
//  } else q_prev100 = queue = queue_top = q;
  ...
  cycles_wo_finds = 0;
  total_bitmap_size += count_bytes(trace_bits);
  ...
  while (i--) {
    ...
    if (unlikely(*current) && unlikely(*current & *virgin)) {
      ...
      for (j = 0; j < sizeof(void*); j++) {
        if (cur[j] & vir[j]) {
#ifdef __x86_64__
          new_bits[new_bits_num * 2] = (MAP_SIZE - ((i + 1) << 3)) + j;
#else
          new_bits[new_bits_num * 2] = (MAP_SIZE - ((i + 1) << 2)) + j;
#endif
          new_bits[new_bits_num * 2 + 1] = cur[j];
          new_bits_num ++;
        }
      }
...
EXP_ST void setup_shm(void) {
  ...
  memset(virgin_bits2, 0, MY_SIZE);
  memset(bc_test_bits,0, MY_SIZE*4);

  memset(virgin_tmout, 255, MAP_SIZE);
  memset(virgin_crash, 255, MAP_SIZE);

  shm_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);
  shm_id2 = shmget(IPC_PRIVATE, MY_SIZE, IPC_CREAT | IPC_EXCL | 0600);
  ...
  trace_bits2 = shmat(shm_id2, NULL, 0);

  if (!trace_bits) PFATAL("shmat() failed");
  if (!trace_bits2) PFATAL("shmat() failed");
...
//  char runner[1024];
  ...
  memset(trace_bits2, 0, MY_SIZE);
  MEM_BARRIER();
  
//  sprintf(runner, runner_fmt, argv[0], argv[1], output);
//  system(runner);
  ...
  if (WEXITSTATUS(status) != 0) {
    // Skip if it is not exit normally
    skip_requested = 1;
  }
...
static bool check_reduced_case(char** argv, u8* mem, u32 len, u32 old_cksum) {
  u8 fault = 0;
...
static u32 unicode_to_raw(wint_t* unicode, u32 unicode_len, u8* raw, u32 len) {
  u8* reduce_fn = alloc_printf("%s/.reduce_path", out_dir);
  u32 reduce_len = 0;
  FILE* fp = fopen(reduce_fn, "wb");

  if (fp == NULL) PFATAL("Unable to create '%s'", reduce_fn);
  for (u32 j = 0; j < unicode_len; j++)
    fputwc(unicode[j], fp);
  fclose(fp);

  // Read again in reduced form
  fp = fopen(reduce_fn, "rb");
  if (fp == NULL) PFATAL("Unable to open '%s'", reduce_fn);
  fseek(fp, 0L, SEEK_END);
  reduce_len = ftell(fp);
  if (reduce_len > len) PFATAL("Buffer is not enough to convert unicode");
  rewind(fp);
  ck_fread(raw, 1, reduce_len, fp, reduce_fn);
  fclose(fp);
...
static void write_coverage_diff() {
    int fd;
    u8* covfn = alloc_printf("%s/.cov_diff", out_dir);
    fd = open(covfn, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    if (fd < 0) PFATAL("Unable to create '%s'", covfn);
    ck_write(fd, &new_bits_num, sizeof(u32), covfn);
    ck_write(fd, new_bits, 2 * new_bits_num * sizeof(u32), covfn);
    close(fd);
    ck_free(covfn);
}
...
static void globally_sync(bool crash) {
  ...
  if (!crash) {
    orig_bitmap = virgin_bits;
    bitmap_name = ".gcov_path";
    cmd = "pathBitmap";
  }
  else {
    orig_bitmap = virgin_crash;
    bitmap_name = ".gcov_crash";
    cmd = "crashBitmap";
  }

  fn = alloc_printf("%s/%s", out_dir, bitmap_name);
  cmdline = alloc_printf("node %s/../TS/redis_ctrl.js downloadBitmap %s %s",
      own_loc, cmd, fn);
  ...
  fd = open(fn, O_RDONLY);
  if (fd < 0) {
    WARNF("Unable to open '%s'", fn);
    goto cleanup;
  }

  ck_read(fd, trace_bits, MAP_SIZE, fn);
  close(fd);

  has_new_bits(orig_bitmap, true);
...
u8 has_new_bc()
{
  ...
  {
    if(trace_bits2[i]==1 && virgin_bits2[i] == 0)
    {
      virgin_bits2[i] = 1;
      found_bc++;
      new_bc=1;
      ...
      cmdline = alloc_printf("node %s/../TS/redis_ctrl.js"
        " insertBC %s %d", own_loc, current_fn, i);
      execute_sh(cmdline);

      cmdline2 = alloc_printf("node %s/../TS/redis_ctrl.js"
        " insertFoundBC %d", own_loc, i);
      execute_sh(cmdline2);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (hnb == 2) {
      ...
      queued_with_cov++;
      // has new edge && preserve old bc
      if(current_bc != -1){
        if(trace_bits2[current_bc]==1){
          cmdline = alloc_printf("node %s/../TS/redis_ctrl.js" " insertBC %s %d", own_loc, current_fn, current_bc);
          execute_sh(cmdline);
          ck_free(cmdline);
        }
      }
...
static void write_wjm_bitmap(){
  ...
  for(int i=0; i<437; i++)
  {
    fprintf(f, "%s: %d\n", DFGIR_list[i], bc_test_bits[i+4+300]);
  }
  fclose(f);
}
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  // https://stackoverflow.com/questions/37248381/how-to-generate-a-host-unique-id
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%0.02f%% / %0.02f%%", 0.0, t_byte_ratio);
...
static void set_own_loc(char* argv0) {
  u8 *rsl;
  own_loc = ck_strdup(argv0);
  rsl = strrchr(own_loc, '/');
...
static s32 fuzz_dir(char* input_dir, char** argv) {
  stage_name  = "javascript";
  stage_short = "js";
  stage_cur   = 0;
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    if (fn_len > 2 && fn[fn_len - 2] == '.' && fn[fn_len - 1] == 't')
        continue;
    // Skip .jsi file
    if (fn_len > 4 && fn[fn_len - 4] == '.' && fn[fn_len - 3] == 'j' && fn[fn_len - 2] == 's' && fn[fn_len - 1] == 'i')
        continue;

    // Skip .raw file
    if (fn_len > 4 && fn[fn_len - 4] == '.' && fn[fn_len - 3] == 'r' && fn[fn_len - 2] == 'a' && fn[fn_len - 1] == 'w')
        continue;
    ...
    // XXX: Should we skip too large file?
    // if (st.st_size > MAX_FILE)
    //   FATAL("Test case '%s' is too big (%s, limit is %s)", fn,
    //        DMS(st.st_size), DMS(MAX_FILE));
...
static s32 generate_js(u8* cur_input, u8* tmp_outdir) {
  ...
  seed = UR(UINT32_MAX);
  ts_dir = alloc_printf("%s/../TS", own_loc);
...
static int compute_less_bc()
{
  // traverse bc_test_bits, exclude 0 -> tmp_bc_test, tmp_bc_num
  tmp_bc_num = 0;
  for(int i=0; i<MY_SIZE; i++)
  {
    if(bc_test_bits[i]!=0)
    {
      tmp_bc_test[tmp_bc_num] = bc_test_bits[i];
      tmp_bc_num++;
    }
  }
  if(tmp_bc_num==0)
  ...
  // sort tmp_bc_test, identify boundary
  qsort(tmp_bc_test, tmp_bc_num, sizeof(int), my_cmp);
  int boundary_id = tmp_bc_num/3;
  int boundary = tmp_bc_test[boundary_id];
  printf("boundary_id: %d\n", boundary_id);
  printf("boundary test num: %d\n", boundary);

  // select bc_id from bc_test_bits according to the boundary
  less_bc_num = 0;
  for(int i=0; i<MY_SIZE; i++)
  {
    if(bc_test_bits[i] && (bc_test_bits[i] < boundary))
    {
      less_bc_id[less_bc_num] = i;
      less_bc_num++;
    }
  }
  return less_bc_num;
...
static u8 fuzz_js(char** argv) {
  ...
  if(less_bc_num!=0)
  {
    ...
    if(pos<50)
    {
      printf("[Corpus] Select seed from bytecode %u corpus\n", less_bc_id[bc_cursor]);
      cmdline = alloc_printf("node %s/../TS/redis_ctrl.js getNextBCTestcase %s %u", own_loc, cur_input, less_bc_id[bc_cursor]);
      current_bc = (int)less_bc_id[bc_cursor];
      bc_cursor++;
    }
    else {
      cmdline = alloc_printf("node %s/../TS/redis_ctrl.js getNextTestcase %s", own_loc, cur_input);
      current_bc = -1;
    }
  }
  else {
...
static void set_fuzzer_id() {
  u8 *fn, *env = getenv("AFL_FUZZER_ID");
  int fd;

  if (env)
    fuzzer_id = ck_strdup(env);
  else {
    const u8* hexdigits = "0123456789abcdef";
    const u8* prefix = "fuzzer-";
    // fuzzer-[8byte hexdigits]
    fuzzer_id =  ck_alloc(strlen(prefix) + 8 + 1);
    strcpy(fuzzer_id, prefix);
    for (int i = strlen(prefix); i < strlen(prefix) + 8; i++)
      fuzzer_id[i] = hexdigits[UR(strlen(hexdigits))];
  }

  // Save fuzzer_id for debugging
  fn = alloc_printf("%s/.fuzzer_id", out_dir);
  fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC, 0600);
  if (fd < 0) PFATAL("Unable to open '%s'", fn);
  ck_write(fd, fuzzer_id, strlen(fuzzer_id), fn);
  close(fd);
  ck_free(fn);
}
...
static void save_exec_cmdline(u32 argc, char** argv, int optind) {
  u8* tmp = orig_cmdline;
  u8* aa_loc;

  save_cmdline(argc - optind, argv + optind);

  exec_cmdline = orig_cmdline;
  orig_cmdline = tmp;

  aa_loc = strstr(exec_cmdline, "@@");
  if (aa_loc == 0)
    PFATAL("@@ is required for fuzzing");

  if (strstr(aa_loc + 2, "@@"))
    PFATAL("Multiple @@ is not allowed");
}
...
int main(int argc, char** argv) {
  ...
  save_exec_cmdline(argc, argv, optind);
  ...
  while (1) {
    if(get_cur_time() - start_time > 1000*60*60*72) // 72h
      goto stop_fuzzing;
    if(iteration_times>1000 && bc_cursor == less_bc_num)
    {
      printf("[Bytecode] Compute less bc....\n");
      compute_less_bc(); 
      bc_cursor=0;
      printf("[Bytecode] Compute less bc finish! Less bc num: %d\n", less_bc_num);
    }

    u8 skipped_fuzz;

    skipped_fuzz = fuzz_js(use_argv);
