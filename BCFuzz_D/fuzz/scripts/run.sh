...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]
  then
      echo "[usage] run.sh [target binary path] [path of DIE-corpus dir] [target js engine (ch/jsc/v8/ffx)]"
    exit 1
fi
...
#./fuzz/scripts/run.sh /home/wjm/0x00Die/smry-die-moz/mozilla/fuzzbuild_OPT.OBJ/dist/bin/js ./corpus ffx
